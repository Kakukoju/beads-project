...
const DASHBOARD_DATA = {
  ...
  scheduleOverview: {
    timelines: [
      { time: "10:30", label: "灌注 Batch-A", duration: 60, color: "bg-blue-500", width: "60%" },
...
const DashboardView = () => {
  ...
  useEffect(() => {
    ...
    // 設定每 2 小時呼叫一次 (2 * 60 * 60 * 1000 = 7200000 ms)
    const interval = setInterval(fetchYieldStats, 7200000);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full animate-in fade-in duration-500">
      <div className="space-y-6">
        ...
        <Card title="生產排程變異" className="h-64">
          <div className="flex flex-col justify-center h-full gap-4 pb-4">
            {scheduleVariance.items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4">
                <span className="text-slate-400 font-mono text-sm w-12 text-right">{item.time}</span>
                <div className="flex-1 bg-slate-700/30 rounded-r-full h-4 relative overflow-hidden">
                  <div className={`absolute top-0 left-0 h-full rounded-r-full ${item.color} opacity-80`} style={{ width: `${item.value}%` }}></div>
                </div>
              </div>
            ))}
          </div>
            ...
            {completionItems.map((item) => (
              <div key={item.id} className="flex flex-col items-center gap-3">
                <div className="relative">
                  ...
                  <div className={`absolute inset-0 rounded-full blur-xl opacity-20 bg-[${item.ringColor}]`}></div>
                </div>
