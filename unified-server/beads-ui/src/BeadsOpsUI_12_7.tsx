...
import { Loader2, Play, Undo2, FileOutput, Settings, Calendar, TerminalSquare, FolderOpen, AlertCircle, Database, FileSpreadsheet, List } from "lucide-react";
...
export default function BeadsOpsUI() {
  ...
  const generateForms = async () => {
    ...
    console.log(`Fetching from DB: ${DB_PATH}, Table: schedule_${year}, Dates:`, dates);
    ...
    setTimeout(() => {
        ...
        dates.forEach(date => {
            ...
            // 模擬資料 (Dummy Records)
            const dummyRecords = [
                { 滴定機: 'Port1', Marker: 'Ca-B', PN: '5714400230', 凍乾機台: '8', 數量: 24000.0, 配藥同仁: 'angela', 日期: date, RD給藥時間: '10:30', 預計滴定時間: '11:00', 預計結束: '22:00', 工單號碼: 'TMRA25L116', Lot: '23025501' },
                { 滴定機: 'Port5', Marker: 'NH3', PN: '5714400185', 凍乾機台: '小台', 數量: 1500, 配藥同仁: '宜蓁', 日期: date, RD給藥時間: '11:30', 預計滴定時間: '12:00', 預計結束: '08:15', 工單號碼: 'TMRA25L120', Lot: '18525501' },
                { 滴定機: 'Port1', Marker: 'MG-AD', PN: '5714400223', 凍乾機台: '9', 數量: 4000, 配藥同仁: 'angela', 日期: date, RD給藥時間: '15:30', 預計滴定時間: '16:00', 預計結束: '15:10', 工單號碼: 'TMRA25L124', Lot: '22325501' }, 
            ];
            ...
            const filledRows = rawStructure.map(row => {
                ...
                if (row.type === 'fixed') { // IVEK
                    targetRecord = dummyRecords.find(r => r['滴定機'] === 'IVEK');
                } else if (row.type === 'port') {
                    const portName = row.label; 
                    const recordsForPort = dummyRecords.filter(r => r['滴定機'] === portName);
                    const isSecondSlot = row.id.endsWith('_2'); 
                    if (!isSecondSlot && recordsForPort.length > 0) targetRecord = recordsForPort[0];
                    else if (isSecondSlot && recordsForPort.length > 1) targetRecord = recordsForPort[1];
                }
                if (targetRecord) {
                    return {
                        ...row,
                        data: {
                            col1: targetRecord['滴定機'], col2: targetRecord['Marker'], col3: targetRecord['PN'], col4: targetRecord['凍乾機台'],
                            col5: parseInt(targetRecord['數量']), col6: targetRecord['配藥同仁'], col7: targetRecord['日期'], 
                            col8: targetRecord['RD給藥時間'], col9: targetRecord['預計滴定時間'], col10: targetRecord['預計結束'],
                            col11: targetRecord['工單號碼'], col12: targetRecord['Lot'], col13: ''
                        }
                    };
                }
                return { ...row, data: { col1: row.label, col2:'', col3:'', col4:'', col5:'', col6:'', col7:'', col8:'', col9:'', col10:'', col11:'', col12:'', col13:'' } };
            });
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 min-h-screen w-full bg-slate-950 text-slate-200">
      ...
      <footer className="text-xs text-slate-500 pt-6 border-t border-slate-800 flex items-center gap-2">
        ...
        v1.6 UI — Fixed: Demand Calculation is now real. Forms Module included.
      </footer>
    </div>
  );
}