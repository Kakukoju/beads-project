...
const DEFAULT_OUTDIR = "\\\\fls341\\MBBU_FAB\\MB_PD\\生管自動化\\滴定\\beadsSchedule";
const DEFAULT_DEMAND_TARGET = "\\\\fls341\\MBBU_FAB\\MB_PD\\生管自動化\\滴定\\beads 需求模組.xlsx";
const DB_PATH = "\\\\fls341\\MBBU_FAB\\MB_PD\\生管自動化\\滴定\\資料庫\\Beads_Schedule.db";
...
const Field: React.FC<FieldProps> = ({ label, children, hint, right }) => (
...
const ModuleCard: React.FC<ModuleCardProps> = ({ name, desc, children, footer, status = "idle" }) => {
...
const getSixDaysArray = (yearStr: string, mmdd: string): string[] => {
  try {
...
// Fixed table structure definition (Row 1-34)
const createEmptyTableStructure = () => {
  ...
  for (let i = 1; i <= 12; i++) {
    rows.push({ type: 'port', label: `Port${i}`, id: `Port${i}_1` });
  }
  ...
  for (let i = 1; i <= 12; i++) {
    rows.push({ type: 'port', label: `Port${i}`, id: `Port${i}_2` });
  }
...
export default function BeadsOpsUI() {
  // ================================
  // State: Tab 1 - Demand Statistics Module
  // ================================
  const [year, setYear] = useState<string>("2025");
  const [dateMMDD, setDateMMDD] = useState<string>("");
  const [scriptPath, setScriptPath] = useState<string>("D:/OneDrive - 天亮醫療器材股份有限公司/.vscode/Bead_auto_update_schedule/plan_to_bead_requirements_1.py");
  ...
  const [vacationStaff, setVacationStaff] = useState<string>("");
  const [schedRunning, setSchedRunning] = useState<boolean>(false);
  const [schedLogs, setSchedLogs] = useState<{ ts: string; msg: string }[]>([]);
  const [schedPreview, setSchedPreview] = useState<any[]>([]);

  // ================================
  // State: Tab 2 - Form Generation System
  // ================================
  ...
  // === Functions: Tab 1 - Demand Statistics ===
  ...
  const toggleHoliday = useCallback((dayOffset: string) => { 
    ...
    setHolidays((prev) => 
      prev.includes(holidayStr) 
        ? prev.filter((h) => h !== holidayStr) 
        : [...prev, holidayStr]
    );
  }, [schedMMDD]);
  ...
  // === Functions: Tab 2 - Forms Generation ===
        ...
        dates.forEach(date => {
          ...
          const filledRows = rawStructure.map(row => {
            ...
            if (row.type === 'fixed') {
              ...
              // Assign first record to IVEK_1, second to IVEK_2
              ...
              // Check if it's the second slot (PM) for this port
            ...
            if (targetRecord) {
              return {
                ...
                data: {
                  ...
                  col12: targetRecord['Lot'], 
                  col13: targetRecord['remark'] || ''
                }
              };
            }
            
            // Return empty row structure with Port label
            return { 
              ...row, 
              data: {
                col1: row.label,
    ...
    <div className="p-6 md:p-10 space-y-8 min-h-screen w-full bg-slate-950 text-slate-200">
      ...
      <Tabs defaultValue="scheduler" className="w-full">
        <TabsList className="grid w-full grid-cols-2 bg-slate-900 border border-slate-800">
          <TabsTrigger value="scheduler" className="data-[state=active]:bg-slate-800 data-[state=active]:text-slate-100 text-slate-400">
            排程產生系統
          </TabsTrigger>
          <TabsTrigger value="forms" className="data-[state=active]:bg-slate-800 data-[state=active]:text-slate-100 text-slate-400">
            表單產生系統
          </TabsTrigger>
        </TabsList>

        {/* ================= Tab 1: Schedule Generation System ================= */}
        <TabsContent value="scheduler" className="space-y-6 mt-4">
          <SectionTitle 
            title="排程產生系統" 
            subtitle="包含兩個模組，每個模組皆有 Input / 執行 / Output / Back" 
          />
                ...
                <div className="grid gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className="text-slate-300 border-slate-600">
                      plan_to_bead_requirements_1.py
                    </Badge>
                  </div>
                  ...
                  <div className="text-xs space-y-1 max-h-40 overflow-y-auto border border-slate-700 p-2 rounded bg-slate-950/50 font-mono text-slate-300">
                    {logEntries.map((e, idx) => (
                      <div key={idx}>
                        <span className="text-slate-500 mr-2">{e.ts}</span>
                        {e.msg}
                      </div>
                    ))}
                  </div>
                        ...
                        <thead>
                          <tr className="bg-slate-800 text-slate-200">
                            {Object.keys(rows[0]).map(h => (
                              <th key={h} className="px-2 py-1 border border-slate-700">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {rows.map((row, idx) => (
                            <tr key={idx} className="hover:bg-slate-800/50">
                              {Object.values(row).map((val: any, i) => (
                                <td key={i} className="px-2 py-1 border border-slate-700">{val}</td>
                              ))}
                            </tr>
                          ))}
                        </tbody>
            ...
            >
              <div className="grid gap-4">
                ...
                <Field label="Date (MM/DD)">
                  <div className="flex gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        const d = new Date();
                        setDateMMDD(`${String(d.getMonth() + 1).padStart(2, "0")}/${String(d.getDate()).padStart(2, "0")}`);
                      }}
                    >
                      今天
                    </Button>
                ...
                >
                  <div className="flex gap-2">
                    <Input 
                      value={demandTargetFile} 
                      onChange={(e) => setDemandTargetFile(e.target.value)} 
                      className="bg-slate-950 border-slate-800 text-slate-100" 
                    />
                    <Button 
                      variant="outline" 
                      className="border-slate-700 text-slate-300 hover:bg-slate-800" 
                      onClick={() => pickOnServer("target_excel", setDemandTargetFile)}
                    >
                ...
                <Field label="Output 結果">
                  <div className="flex items-center gap-2 w-full">
                    <Input 
                      value={fileName || ""} 
                      readOnly 
                      className="truncate bg-slate-950/50 border-slate-800 text-slate-400" 
                      placeholder="尚未執行" 
                    />
                    {fileName && (
                      <Badge 
                        ...
                        onClick={copyPath} 
                        variant="outline" 
                        className="cursor-pointer hover:bg-slate-800 text-slate-300 border-slate-600"
                      >
                        {copiedAt ? "已複製" : "複製路徑"}
                      </Badge>
                    )}
                ...
                <Field 
                  label="Dry Run" 
                  right={dryRun ? <span className="text-xs text-slate-400">不寫回，只產生預覽</span> : undefined}
                >
                  <Switch 
                    checked={dryRun} 
                ...
                <div className="flex justify-end pt-2">
                  <Button 
                    onClick={runDemand} 
                    disabled={running || !year || !dateMMDD || !scriptPath} 
              ...
              footer={
                <div className="grid gap-3">
                  ...
                  <div className="text-xs space-y-1 max-h-40 overflow-y-auto border border-slate-700 p-2 rounded bg-slate-950/50 font-mono text-slate-300">
                    {schedLogs.map((e, idx) => (
                      <div key={idx}>
                        <span className="text-slate-500 mr-2">{e.ts}</span> {e.msg}
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid gap-4">
                <Field label="當週第一天 (MM/DD)" hint="依序產生 6 天排程">
                  <div className="flex gap-2">
                    <Input 
                      value={schedMMDD} 
                      onChange={(e) => setSchedMMDD(e.target.value)} 
                      className="bg-slate-950 border-slate-800 text-slate-100" 
                      placeholder="MM/DD (從左側連動)" 
                    />
                    <Button 
                      variant="outline" 
                      className="border-slate-700 text-slate-300 hover:bg-slate-800" 
                      onClick={() => {
                        const d = new Date();
                        setSchedMMDD(`${String(d.getMonth() + 1).padStart(2, "0")}/${String(d.getDate()).padStart(2, "0")}`);
                      }}
                    >
                      <Calendar className="w-4 h-4 mr-2" /> 今天
                    </Button>
                  </div>
                  {validSchedMMDD && (
                    <div className="text-xs text-emerald-500 mt-1 pl-1">✓ 日期格式正確</div>
                  )}
                </Field>
                ...
                <Field label="需求檔來源" hint="只需按右側圖示選檔，路徑會自動填入">
                  <div className="flex items-center gap-2">
                    <Input 
                      ...
                      placeholder="尚未選擇（將從左側模組連動）" 
                    />
                    <Button 
                      variant="outline" 
                      className="border-slate-700 text-slate-300 hover:bg-slate-800" 
                      type="button" 
                      onClick={() => pickOnServer("need", setSchedNeedPath)}
                    >
                ...
                <Field 
                  label="本周休假日" 
                  hint={holidays.length > 0 ? `已選: ${holidays.join(', ')}` : "請先輸入當週第一天"}
                >
                      ...
                      return (
                        <div key={day.value} className="flex items-center space-x-2">
                          <Checkbox 
                            ...
                            disabled={!schedMMDD || !validSchedMMDD} 
                            className="border-slate-600 data-[state=checked]:bg-slate-100 data-[state=checked]:text-slate-900" 
                          />
                          <label 
                            htmlFor={`holiday-${day.value}`} 
                ...
                <Field label="本周休假人員" hint="格式範例：11/10-張三,11/11-李四">
                  <Input 
                    value={vacationStaff} 
                    onChange={(e) => setVacationStaff(e.target.value)} 
                    placeholder="例如：11/10-張三,11/11-李四" 
                    className="bg-slate-950 border-slate-800 text-slate-100" 
                  />
                </Field>
                ...
                <Field label="輸出資料夾" hint="預設=MBBU_FAB\MB_PD\生管自動化\滴定\beadsSchedule">
                  <div className="flex items-center gap-2">
                    <Input 
                      ...
                      readOnly 
                      className="bg-slate-950/50 border-slate-800 text-slate-400" 
                      placeholder={DEFAULT_OUTDIR} 
                    />
                    <Button 
                      variant="outline" 
                      className="border-slate-700 text-slate-300 hover:bg-slate-800" 
                      type="button" 
                      onClick={() => pickOnServer("outdir", setOutDir)}
                    >
                ...
                <Field 
                  label="Dry Run" 
                  right={dryRun ? <span className="text-xs text-slate-400">不另存，僅預覽</span> : undefined}
                >
                  <Switch 
                    checked={dryRun} 
                    onCheckedChange={setDryRun} 
                ...
                <div className="flex gap-2 justify-end pt-2">
                  ...
                  <Button 
                    onClick={runSchedule} 
                    disabled={schedRunning || !canRunSchedule} 
                    className="bg-slate-100 text-slate-900 hover:bg-slate-200"
                  >
                    {schedRunning ? (
                      <>
        ...
        {/* ================= Tab 2: Form Generation System ================= */}
        <TabsContent value="forms" className="space-y-6 mt-4">
          <SectionTitle 
            title="表單產生系統" 
            subtitle="從資料庫讀取排程資料，並產生每日生產表單" 
          />
          ...
          <Card className="rounded-xl shadow-lg bg-slate-900 border-slate-800">
            <CardContent className="p-5 flex flex-col md:flex-row gap-6 items-start md:items-end">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 w-full">
                <div className="space-y-2">
                  <Label className="text-slate-300 flex items-center gap-2">
                    <Database className="w-4 h-4 text-blue-400" /> 資料庫來源 (DB Path)
                  </Label>
                  <Input 
                    value={DB_PATH} 
                    readOnly 
                    className="bg-slate-950/50 border-slate-700 text-slate-400 text-xs font-mono" 
                  />
                </div>
                
                <div className="space-y-2">
                  <Label className="text-slate-300 flex items-center gap-2">
                    <List className="w-4 h-4 text-green-400" /> 資料表 (Table)
                  </Label>
                  <Input 
                    value={`schedule_${year}`} 
                    readOnly 
                    className="bg-slate-950/50 border-slate-700 text-slate-400 text-xs font-mono" 
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label className="text-slate-300 flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-yellow-400" /> 起始日期 (Start Date)
                  </Label>
                  <Input 
                    value={validSchedMMDD ? `${year}/${schedMMDD} (共6天)` : "請先在 [排程產生系統] 設定日期"} 
                    readOnly 
              ...
              <Button 
                ...
                disabled={formsLoading || !validSchedMMDD} 
                className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white min-w-[140px]"
              >
                {formsLoading ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" /> 
                  ...
                  <TabsList className="bg-slate-900 border border-slate-800 h-auto p-1 flex-wrap justify-start">
                    {formDates.map(date => (
                      <TabsTrigger 
                        key={date} 
                        value={date} 
                        className="data-[state=active]:bg-blue-900 data-[state=active]:text-blue-100 text-slate-400 text-xs px-3 py-1.5"
                      >
                        {date}
                      </TabsTrigger>
                    ))}
                  </TabsList>
                  <div className="text-xs text-slate-500">顯示 Row 1~34 (固定格式)</div>
                </div>
                          ...
                          {formsData[date]?.map((row, idx) => {
                            if (row.type === 'empty') {
                              return (
                                <tr key={idx} className="h-6">
                                  <td colSpan={13} className="border border-slate-300 bg-slate-50"></td>
                                </tr>
                              );
                            }
                            ...
                            return (
                              <tr key={idx} className="h-6 hover:bg-blue-50 transition-colors">
                                <td className="border border-slate-400 px-1 text-center font-semibold bg-slate-100">{d.col1}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col2}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col3}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col4}</td>
                                <td className="border border-slate-400 px-1 text-right font-mono">{d.col5}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col6}</td>
                                <td className="border border-slate-400 px-1 text-center whitespace-nowrap">{d.col7}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col8}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col9}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col10}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col11}</td>
                                <td className="border border-slate-400 px-1 text-center">{d.col12}</td>
                                <td className="border border-slate-400 px-1">{d.col13}</td>
                              </tr>
                            );
                          })}
                        </tbody>
      ...
      <footer className="text-xs text-slate-500 pt-6 border-t border-slate-800 flex items-center gap-2">
        ...
        v2.0 Fixed — All TypeScript errors resolved
      </footer>
    </div>
  );
}