...
const getWeekDateString = (baseMMDD: string, dayOffset: string): string | null => {
  try {
    ...
    targetDate.setDate(monday.getDate() + parseInt(dayOffset) - 1); // dayOffset 是 1-based
...
export default function BeadsOpsUI() {
  ...
  const copyPath = async () => {
    ...
    try {
      // navigator.clipboard.writeText is preferred but might fail in some envs
      if (navigator.clipboard && navigator.clipboard.writeText) {
        await navigator.clipboard.writeText(writeBackPath);
      } else {
        // Fallback for insecure contexts or older browsers
        const textarea = document.createElement('textarea');
        textarea.value = writeBackPath;
  ...
  const [schedNeedPath, setSchedNeedPath] = useState<string>(""); // ✅ 會從模組 1 連動
  ...
  // 當週第一天
  const [schedMMDD, setSchedMMDD] = useState<string>(""); // ✅ 會從模組 1 連動
  ...
  // ✅ 更新：toggleHoliday 使用輔助函數
  const toggleHoliday = useCallback((dayOffset: string) => {
    if (!schedMMDD) return; // 使用排程模組的日期
  ...
  const canRunSchedule = useMemo(() => {
    ...
    // ❌ 移除 SchedLimitPath 和 tmplPath 檢查
    ...
    <div className="p-6 md:p-10 space-y-8 min-h-screen w-full">
            ...
            <ModuleCard
              ...
              footer={<div className="grid gap-3">
                <div className="flex items-center justify-between">
                  ...
                  {/* ✅ 更新腳本名稱 */}
                  <Badge variant="outline">beads_Scheduler_V9_9_7.py</Badge>
                </div>
                <div className="text-xs space-y-1 max-h-40 overflow-y-auto border p-2 rounded bg-muted/30">
                  {schedLogs.map((e, idx) => (
                    <div key={idx}>
            ...
            >
              <div className="grid gap-4">
                ...
                <Field label="當週第一天 (MM/DD)" hint="依序產生 6 天排程">
                  <div className="flex gap-2">
                    <Input value={schedMMDD} onChange={(e) => setSchedMMDD(e.target.value)} placeholder="MM/DD (從左側連動)" />
                    <Button
                      variant="outline"
                      type="button"
                      onClick={() => {
                  ...
                  {validSchedMMDD && (
                    <div className="text-xs text-emerald-600 mt-1">✓ 已從左側模組連動日期</div>
                  )}
                </Field>
                ...
                <Field label="需求檔來源" hint="只需按右側圖示選檔，路徑會自動填入">
                  <div className="flex items-center gap-2">
                    <Input value={schedNeedPath} readOnly className="bg-muted/50" placeholder="尚未選擇（將從左側模組連動）" />
                  ...
                  {/* ✅ 更新：連動提示 */}
                  {schedNeedPath && (
                    <div className="text-xs text-emerald-600 mt-1">✓ 已從左側模組連動路徑</div>
                  )}
                </Field>
                ...
                {/* ✅ 新增：休假人員 */}
                <Field label="本周休假人員" hint="格式：MM/DD-人名，多筆用逗號分隔">
                  <Input
                    value={vacationStaff}
                ...
                <Field label="輸出資料夾" hint="預設=MBBU_FAB\MB_PD\生管自動化\滴定\beadsSchedule；按下可更改資料夾">
                  <div className="flex items-center gap-2">
                    <Input value={outDir} readOnly className="bg-muted/50" placeholder={DEFAULT_OUTDIR} />
      ...
      <footer className="text-xs text-muted-foreground pt-6 border-t">
        <div className="flex items-center gap-2">
          ...
          {/* ✅ 更新版本說明 */}
          v0.9 UI — 模組 1 (v0.7)；模組 2 (vFinal - 連動日期/路徑，預設週六日休假，移除模板)
        </div>
      </footer>
    </div>
  );
