...
from process_analysis_blueprint import process_analysis_bp
from wip_automation_blueprint_1 import (wip_automation_bp, init_wip_automation)
from sync_upload_blueprint import sync_upload_bp
...
APP_DIR = "/opt/beadsops/unified-server"
...
app.register_blueprint(sync_upload_bp)
    ...
    try:
        ...
        with sqlite3.connect(FORMULATE_DB_PATH) as conn:
            ...
            if search_type == "week":
                ...
                # --- 🔥 修改重點：日期搜尋 (支援三種格式) ---
                # 產生所有可能的日期變體
                date_variants = generate_date_variants(search_value)
                
                # 動態產生 SQL: WHERE Date IN (?, ?, ?)
                placeholders = ",".join(["?"] * len(date_variants))
                query = f"SELECT * FROM DropletSchedule WHERE Date IN ({placeholders})"
                params = tuple(date_variants)

                logger.info(f"🔎 搜尋日期變體: {date_variants}")
...
# ====================================================================
# WIP Excel Sync API (VBA)
# ====================================================================
@app.route("/api/wip/sync-excel", methods=["POST"])
def api_wip_sync_excel():
    """
    VBA sync endpoint.
    mode=full  : batch 1 = DROP+CREATE, batch>=2 = append  (全量上傳)
    mode=upsert: DELETE by LOT NO then INSERT               (差異上傳)
    Payload: { "table":"明細_2025", "mode":"full|upsert", "batch":1, "rows":[...] }
    """
    import sqlite3 as _sqlite3
    WIP_SYNC_DB = "/opt/beadsops/data/Bead_Sort_DB.db"
    KEY_COL = "LOT NO"
    try:
        ...
        if not rows:
            return jsonify({"ok": False, "message": "No rows"}), 400

        cols = list(rows[0].keys())
        col_defs = ", ".join([f'"{c}" TEXT' for c in cols])

        with _sqlite3.connect(WIP_SYNC_DB, timeout=30) as conn:
            ...
            if mode == "full":
                if batch == 1:
                    ...
                    logger.info(f"WIP Sync full batch 1: DROP+CREATE {table}")
                else:
                ...
                keys = [str(r.get(KEY_COL, "")) for r in rows if r.get(KEY_COL)]
                if keys:
                    ph = ",".join(["?"] * len(keys))
                    conn.execute(f'DELETE FROM "{table}" WHERE "{KEY_COL}" IN ({ph})', keys)
                    logger.info(f"WIP Sync upsert: deleted {len(keys)} keys before insert")
            ...
            for row in rows:
                values = [str(row.get(c, "")) if row.get(c) is not None else "" for c in cols]
                conn.execute(sql, values)

            conn.commit()

        logger.info(f"WIP Sync {mode} batch {batch}: {len(rows)} rows -> {table}")
        return jsonify({"ok": True, "mode": mode, "batch": batch, "rows": len(rows),
                        "message": f"{mode} batch {batch}: {len(rows)} rows synced"})

    except Exception as e:
        logger.error(f"WIP Sync failed: {e}")
        return jsonify({"ok": False, "message": str(e)}), 500
