...
# ====================================================================
# 修正版排程系統 (新增CP Solver支援)
# ====================================================================
USE_CP_SOLVER = False  # 設為 False 則使用貪婪算法
if USE_CP_SOLVER:
    try:
        from scheduler_integration import run_cp_scheduler_integrated
        print("✅ CP 排程器模組載入成功", file=sys.stderr)
    except ImportError as e:
        print(f"⚠️ CP 排程器模組載入失敗: {e}", file=sys.stderr)
        print("   將使用貪婪算法", file=sys.stderr)
        USE_CP_SOLVER = False
# --------------------------------------------------------------------
# 1. 設定
# --------------------------------------------------------------------
...
# ============================================================================
# 修改 1: calculate_demand_queue - 只计算 P1-P3
# ============================================================================

def calculate_demand_queue(data: dict):
    print("Step 2: 計算 P1-P3 需求優先級 (僅排核心任務)...", file=sys.stderr)
    ...
    for _, r in df_grouped.iterrows():
        ...
        if pd.isna(prod_qty) or prod_qty == 0: 
            print(f"  ⚠️ 警告: 群組 {group_name} 的 BDC_Prod_Qty 為 0 或 NaN，跳過。", file=sys.stderr)
            continue
        ...
        if w1_short > 0:
            ...
            p1_log.append(f"{representative_pn} ({group_name}) - 缺 {w1_short}")
        
        # P2: W2 缺料
        if w2_short > 0:
            subp = (dryer_prio, person_prio, 0)
            task_queue.append((2, subp, representative_pn, w2_short, "W2", group_name, marker_name, prod_qty))
            p2_log.append(f"{representative_pn} ({group_name}) - 缺 {w2_short}")
        
        # P3: W3 缺料
        if w3_short > 0:
            subp = (dryer_prio, person_prio, 0)
            task_queue.append((3, subp, representative_pn, w3_short, "W3", group_name, marker_name, prod_qty))
            p3_log.append(f"{representative_pn} ({group_name}) - 缺 {w3_short}")

    # 列印 P1-P3 清單
    print("-" * 50, file=sys.stderr)
    print("📊 核心需求清單 (P1-P3):", file=sys.stderr)
    
    if p1_log:
        print(f"  🔴 [P1] W1 急缺 ({len(p1_log)} 筆):", file=sys.stderr)
        for item in p1_log: print(f"     - {item}", file=sys.stderr)
    else:
        print("  ✅ [P1] W1 無缺料", file=sys.stderr)

    if p2_log:
        print(f"  🟠 [P2] W2 缺料 ({len(p2_log)} 筆):", file=sys.stderr)
        for item in p2_log: print(f"     - {item}", file=sys.stderr)
    
    if p3_log:
        print(f"  🟡 [P3] W3 缺料 ({len(p3_log)} 筆):", file=sys.stderr)
        for item in p3_log: print(f"     - {item}", file=sys.stderr)
    print("-" * 50, file=sys.stderr)
    ...
    try:
        ...
        if min_stock_r1 < rule1_threshold:
            ...
            if rule1_group_name not in existing_task_groups:
                task = create_p3_stock_task('5714400197', rule1_group_name, pn_info_r1['BDC_Marker_Name'], 
                                           pn_info_r1['BDC_Prod_Qty'], pn_info_r1.get('V9_Dryer_Count', 99), 
                                           pn_info_r1.get('V9_Person_Count', 99))
                if task: 
    ...
    # 規則 2: IVEK-Na (5714400202, 5714400203) < 50000
    ...
    try:
        ...
        if min_stock_r2 < rule2_threshold:
            ...
            if rule2_group_name not in existing_task_groups:
                task = create_p3_stock_task('5714400202', rule2_group_name, pn_info_r2['BDC_Marker_Name'], 
                                           pn_info_r2['BDC_Prod_Qty'], pn_info_r2.get('V9_Dryer_Count', 99), 
                                           pn_info_r2.get('V9_Person_Count', 99))
                if task: 
                    task_queue.append(task)
    ...
    # 排序
    task_queue.sort(key=lambda x: (x[0], x[5], x[1][0], x[1][1], x[1][2], x[2]))
    task_dict = {(t[5], t[2]): t for t in task_queue}

    print(f"✅ 需求計算完成: {len(task_queue)} 個 P1-P3 任務\n", file=sys.stderr)

    return task_queue, task_dict, []  # 返回空的 P6/P7 列表
...
class Scheduler:
    def __init__(self, data):
        ...
        # ✅ 修改：不 接收 P6/P7 任務清單
        self.task_queue, self.task_dict, _ = calculate_demand_queue(data)
        ...
        self.base_monday = SCHEDULE_START_DATE
    ...
    def run(self):
        """
        ✅ 完整修正版: 只排 P0-P3 核心任務，支援 CP-SAT 和貪婪演算法
        """
        import sys
        from datetime import timedelta, datetime
        
        print(f"\n{'='*70}", file=sys.stderr)
        print(f"🚀 開始排程流程 (P0-P3 核心任務)", file=sys.stderr)
        print(f"{'='*70}\n", file=sys.stderr)
        ...
        if SCHEDULE_START_DATE is None:
            today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            monday = (today + timedelta(days=-today.weekday(), weeks=0))
            print(f"  ⚠️ 警告: SCHEDULE_START_DATE 未設定，使用今天推算週一", file=sys.stderr)
        else:
            monday = SCHEDULE_START_DATE
        
        print(f"  📅 排程起始: {monday.strftime('%Y-%m-%d (%A)')}", file=sys.stderr)
        print(f"  📊 排程天數: {self.days_to_schedule} 天\n", file=sys.stderr)

        # 顯示休假日
        if HOLIDAYS:
            print(f"  🗓️ 休假日設定:", file=sys.stderr)
            for h in sorted(HOLIDAYS):
                print(f"      • {h.strftime('%Y-%m-%d (%A)')}", file=sys.stderr)
        ...
        print(f"⚙️ 初始化資源...", file=sys.stderr)
        ...
        for day_index in range(self.days_to_schedule):
            ...
            if is_holiday(current_day):
                print(f"  ↳ 跳過休假日: {current_day.strftime('%Y-%m-%d')}", file=sys.stderr)
                continue
        ...
        print(f"  ✅ 已生成 {self.days_to_schedule} 個排程日\n", file=sys.stderr)
        ...
        print("="*70, file=sys.stderr)
        print("📋 Step 3.1: 處理 P0 強制任務", file=sys.stderr)
        print("="*70, file=sys.stderr)
        
        try:
            self.handle_p0_orders()
            
            p0_strict_count = len(self.p0_strict_records) if hasattr(self, 'p0_strict_records') else 0
            p0_exp_count = len(self.p0_experiment_records) if hasattr(self, 'p0_experiment_records') else 0
            
            print(f"  ✅ P0 處理完成", file=sys.stderr)
            print(f"    • P0 (非實驗): {p0_strict_count} 筆", file=sys.stderr)
            print(f"    • P0 (實驗): {p0_exp_count} 筆\n", file=sys.stderr)
            
        except SystemExit as e:
            print(f"\n❌ P0 處理終止: {str(e)}", file=sys.stderr)
            return
        ...
        print("="*70, file=sys.stderr)
        print("📋 Step 3.2: 處理無凍乾機任務", file=sys.stderr)
        print("="*70, file=sys.stderr)
        
        self.handle_no_dryer_tasks()
        
        no_dryer_count = len(self.no_dryer_records) if hasattr(self, 'no_dryer_records') else 0
        print(f"  ✅ 無凍乾機處理完成: {no_dryer_count} 筆\n", file=sys.stderr)

        # ========================================
        # Step 5: 檢查是否使用 CP 排程器
        # ========================================
        
        print("="*70, file=sys.stderr)
        print(f"🔍 Step 3.3: 檢查排程引擎", file=sys.stderr)
        print("="*70, file=sys.stderr)
        print(f"  USE_CP_SOLVER = {USE_CP_SOLVER}\n", file=sys.stderr)
        
        if USE_CP_SOLVER:
            print("="*70, file=sys.stderr)
            print("🧠 進入 CP-SAT 排程模式", file=sys.stderr)
            print("="*70, file=sys.stderr)
            
            try:
                from scheduler_integration import run_cp_scheduler_integrated
                print("  ✅ 成功導入 run_cp_scheduler_integrated", file=sys.stderr)
                
            except ImportError as e:
                print(f"  ❌ 導入失敗: {e}", file=sys.stderr)
                print(f"  ⚠️ 回退到貪婪算法\n", file=sys.stderr)
                
            else:
                # 調用 CP 排程器
                print(f"  ⚙️ 調用 run_cp_scheduler_integrated(self)...\n", file=sys.stderr)
                
                cp_success = run_cp_scheduler_integrated(self)
                
                print(f"\n  📊 CP 排程器返回值: {cp_success}", file=sys.stderr)
                
                if cp_success:
                    print("\n" + "="*70, file=sys.stderr)
                    print("✅ CP 排程成功完成", file=sys.stderr)
                    print("="*70 + "\n", file=sys.stderr)
                    return  # CP 成功，直接返回
                else:
                    print("\n" + "="*70, file=sys.stderr)
                    print("⚠️ CP 排程失敗，回退到貪婪算法", file=sys.stderr)
                    print("="*70 + "\n", file=sys.stderr)
        else:
            print("  ℹ️ 使用貪婪演算法（CP 已停用）\n", file=sys.stderr)
        ...
        print("="*70, file=sys.stderr)
        print("🏃 Step 3.4: 執行貪婪演算法 (P1-P3)", file=sys.stderr)
        print("="*70, file=sys.stderr)

        print("  ⚙️ 計算任務優先級...", file=sys.stderr)

        # 🔥 診斷 task_queue 結構（調試完可移除）
        if self.task_queue and False:  # 設為 True 啟用診斷
            print("\n🔍 診斷 task_queue 結構:", file=sys.stderr)
            sample = self.task_queue[0]
            print(f"  第一個元素: {sample}", file=sys.stderr)
            for i, item in enumerate(sample):
                print(f"  [{i}]: {type(item).__name__} = {item}", file=sys.stderr)

        # 🔥 修正：根據實際結構排序
        # task = (priority, extra_tag, pn, qty_short, tag, marker_name, group_name, bdc_qty)
        #        [0]       [1]        [2]  [3]        [4]  [5]          [6]         [7]
        
        def get_sort_key(task):
            """根據實際 task_queue 結構提取排序鍵"""
            try:
                priority = task[0]        # int: 優先級
                pn = task[2]              # str: 料號
                qty_short = task[3]       # int: 缺貨數量
                group_name = task[6] if len(task) > 6 else task[5]  # str: 群組名稱
                
                # GLIPA 檢查
                is_glipa = 0 if (hasattr(self, 'RULE_GLIPA_PNs') and pn in self.RULE_GLIPA_PNs) else 1
                
                return (
                    priority,      # 優先級
                    is_glipa,      # GLIPA 優先
                    -qty_short,    # 缺貨量大優先（負數使大的排前面）
                    group_name     # 群組名稱
                )
                
            except Exception as e:
                print(f"  ⚠️ 排序錯誤: {e}, task={task}", file=sys.stderr)
                return (99, 1, 0, "")

        # 執行排序
        sorted_queue = sorted(self.task_queue, key=get_sort_key)

        print(f"  ✅ 任務排序完成: {len(sorted_queue)} 個任務\n", file=sys.stderr)
        ...
        for task in sorted_queue:
            ...
            if priority == 1:
                pn = task[2]
                group_name = task[6] if len(task) > 6 else task[5]
                p1_tasks_set.add((group_name, pn))

        p1_total = len(p1_tasks_set)
        print(f"  📊 P1 任務總數: {p1_total}\n", file=sys.stderr)
        ...
        for task in sorted_queue:
            ...
            # task = (priority, extra_tag, pn, qty_short, tag, marker_name, group_name, bdc_qty)
            priority = task[0]        # int: 優先級
            extra_tag = task[1]       # tuple: (?, ?, ?)
            pn = task[2]              # str: 料號
            qty_short = task[3]       # int: 缺貨數量
            tag = task[4]             # str: 標籤
            marker_name = task[5]     # str: 藥名
            group_name = task[6]      # str: 群組名稱
            bdc_qty = task[7]         # float: BDC 數量
            
            # 如果 group_name 和 marker_name 一樣，使用 marker_name
            if group_name == marker_name:
                group_name = marker_name

            # 取得約束
            if pn not in self.constraints.index:
                print(f"  ⚠️ 跳過: {pn} ({marker_name}) - 無約束資料", file=sys.stderr)
                failed_reasons[pn] = "無約束資料"
                failed_count += 1
                continue

            c_row = self.constraints.loc[pn]
            if isinstance(c_row, pd.DataFrame):
                c_row = c_row.iloc[0]

            # Port 數量檢查
            port_str = str(c_row.get("Port數", "2")).strip()
            is_ivek = (port_str.upper() == "IVEK")
            num_ports = 0 if is_ivek else int(port_str)

            # 可用凍乾機
            dryer_str = str(c_row.get("可用凍乾機", "")).strip()
            usable_dryers = [d.strip() for d in dryer_str.split(',') if d.strip()] if dryer_str else []

            if not usable_dryers:
                print(f"  ⚠️ 跳過: {marker_name} (PN: {pn}) - 無可用凍乾機", file=sys.stderr)
                failed_reasons[pn] = "無可用凍乾機"
                failed_count += 1
                continue
            ...
            freeze_hrs = float(c_row.get("凍乾時間", 12.0))

            # 取得可用人員
            usable_people = []
            for key in ["配藥人-1", "配藥人-2", "配藥人-3"]:
                person = c_row.get(key)
                if person and str(person).strip():
                    usable_people.append(str(person).strip())

            if not usable_people:
                print(f"  ⚠️ 跳過: {marker_name} (PN: {pn}) - 無可用人員", file=sys.stderr)
                failed_reasons[pn] = "無可用人員"
                failed_count += 1
                continue
            ...
            for day_index in range(self.days_to_schedule):
                ...
                for shift_name in ["AM", "PM"]:
                    ...
                    shift_key = f"{day_key}_{shift_name}"

                    # 嘗試分配
                    result = self.try_allocate_task(
                        day_key=day_key,
                    ...
                    if result:
                        ...
                        # 記錄 P1 成功
                        if priority == 1:
                            p1_scheduled.add((group_name, pn))
                        
                        break

            if not scheduled:
                reason = "無可用時段或資源"
                failed_reasons[pn] = reason
                failed_count += 1
                
                if priority <= 1:
                    print(f"  ⚠️ P{priority} 排程失敗: {marker_name} (PN: {pn}) - {reason}", file=sys.stderr)

        print(f"\n  📊 排程循環完成:", file=sys.stderr)
        print(f"    • 成功排程: {scheduled_count} 個任務", file=sys.stderr)
        print(f"    • 排程失敗: {failed_count} 個任務\n", file=sys.stderr)
        ...
        print("="*70, file=sys.stderr)
        print("📊 Step 3.5: P1 任務完成檢查", file=sys.stderr)
        print("="*70, file=sys.stderr)

        p1_scheduled_count = len(p1_scheduled)

        print(f"  P1 總數: {p1_total}", file=sys.stderr)
        print(f"  P1 已排程: {p1_scheduled_count}", file=sys.stderr)
        print(f"  P1 未排程: {p1_total - p1_scheduled_count}\n", file=sys.stderr)
        ...
        if p1_total > 0 and p1_scheduled_count < p1_total:
            ...
            # 列出失敗任務
            print(f"\n  未排程的 P1 任務:\n", file=sys.stderr)
            for group_name, pn in sorted(p1_tasks_set - p1_scheduled):
                reason = failed_reasons.get(pn, "未知原因")
                print(f"    • {group_name} (PN: {pn})", file=sys.stderr)
        ...
        print("="*70, file=sys.stderr)
        print("✅ 貪婪演算法排程完成", file=sys.stderr)
        print("="*70, file=sys.stderr)
        
        # 統計 P2, P3
        all_scheduled_pns = set()
        if hasattr(self, 'schedule_df') and not self.schedule_df.empty:
            for _, row in self.schedule_df.iterrows():
                pn = row.get('lot', '')
                if pn:
                    all_scheduled_pns.add(pn)
        
        p2_count = sum(1 for task in sorted_queue if task[0] == 2 and task[2] in all_scheduled_pns)
        p3_count = sum(1 for task in sorted_queue if task[0] == 3 and task[2] in all_scheduled_pns)
        
        print(f"\n  📊 排程統計:", file=sys.stderr)
        print(f"    • P1: {p1_scheduled_count}/{p1_total} ✅", file=sys.stderr)
        print(f"    • P2: {p2_count} 個", file=sys.stderr)
        print(f"    • P3: {p3_count} 個\n", file=sys.stderr)
        
        # 最終輸出
        self._finalize_and_output()

    def try_allocate_task(self, day_key, shift_name, num_ports, is_ivek, 
                      usable_dryers, dosing_hrs, freeze_hrs, usable_people,
                      group_name, pn, marker_name, prod_qty, priority):
        """
        嘗試為任務分配資源並記錄
        適配原程式的 book_and_record API
        
        Returns:
            bool: 分配成功返回 True，失敗返回 False
        """
        import sys
        ...
        for person in usable_people:
            can_use, reason = self.resources.can_use_person(
                person, shift_key, day_key, is_single_slot, priority
            )
        ...
        for dryer in usable_dryers:
            ...
            if can_use:
                selected_dryer = dryer
                break
        
        if not selected_dryer:
            return False
        ...
        if is_ivek:
            # 檢查 P0 是否佔用 IVEK
            if self.resources.is_p0_strict_ivek_booked(day_key):
                return False
            
            # P3-P5 還要檢查實驗 P0
            if priority >= 3 and self.resources.is_p0_experiment_ivek_booked(day_key):
                return False
        
        # ========================================
        # Step 4: Port 檢查（非 IVEK）
        # ========================================
        
        if not is_ivek:
            # 使用 shift_port_counter 檢查 Port
            if shift_key not in self.shift_port_counter:
                self.shift_port_counter[shift_key] = 1
            
            start_port = self.shift_port_counter[shift_key]
            
            # 檢查 Port 範圍
            if start_port + num_ports - 1 > 12:
                return False
            
            # 檢查這些 Port 是否被 P0 佔用
            for port_num in range(start_port, start_port + num_ports):
                if self.resources.is_p0_strict_port_booked(shift_key, port_num):
                    return False
                
                if priority >= 3 and self.resources.is_p0_experiment_port_booked(shift_key, port_num):
                    return False
        ...
        try:
            # 預訂人員和凍乾機
            self.resources.book_person(selected_person, shift_key, day_key, is_single_slot, priority)
            self.resources.book_dryer(selected_dryer, day_key, priority)
            
        except Exception as e:
            print(f"    ⚠️ 資源預訂失敗: {e}", file=sys.stderr)
            return False
        
        # ========================================
        # Step 6: 構造參數並調用 book_and_record
        # ========================================
        
        # 將 day_key 轉換為 datetime 對象
        current_day = datetime.strptime(day_key, '%Y-%m-%d')
        
        # 計算時間
        # 假設 AM: 08:00 開始，PM: 13:00 開始
        if shift_name == "AM":
            delivery_dt = current_day.replace(hour=8, minute=0)
        else:  # PM
            delivery_dt = current_day.replace(hour=13, minute=0)
        
        dosing_start_dt = delivery_dt + timedelta(hours=0.5)
        dosing_end_dt = dosing_start_dt + timedelta(hours=dosing_hrs)
        final_end_dt = dosing_end_dt + timedelta(hours=freeze_hrs)
        ...
        resources = {
            "times": {
                ...
                "dosing_end_dt": dosing_end_dt,
                "final_end_dt": final_end_dt,
                "freeze_duration_hr": freeze_hrs
            },
            "dryer": selected_dryer,
            "person": selected_person,
            "num_ports_str": "IVEK" if is_ivek else str(num_ports),
            "is_ivek": is_ivek,
            "num_ports_per_pn": 0 if is_ivek else num_ports,
            "is_glipa": False
        }
        
        # 構造 batch_tasks（單任務）
        # 格式: (week, subp, pn, short_qty, tag, group_name, marker_name, prod_qty)
        batch_tasks = [
            (0, 0, pn, 0, "", group_name, marker_name, prod_qty)
        ]
        
        # 調用原有的 book_and_record
        try:
            self.book_and_record(resources, batch_tasks, current_day, shift_name, priority)
            return True
            
        except Exception as e:
            print(f"    ⚠️ 記錄失敗: {e}", file=sys.stderr)
            import traceback
            traceback.print_exc(file=sys.stderr)
            return False
    ...
    def _finalize_and_output(self):
        ...
        try:
            ...
            if not os.path.exists(output_dir):
                try:
                    ...
                    print(f"  ℹ️ 建立輸出目錄: {output_dir}", file=sys.stderr)
                except Exception as e:
                    print(f"  ⚠️ 無法建立目錄: {e}", file=sys.stderr)
            
            # 檢查寫入權限
            if not os.access(output_dir, os.W_OK):
                print(f"  ⚠️ 輸出目錄無寫入權限: {output_dir}", file=sys.stderr)
                raise PermissionError(f"無法寫入目錄: {output_dir}")
            
            # 調用輸出方法
            self.save_records_to_excel(outdir=output_dir)
            print(f"  ✅ Excel 輸出成功", file=sys.stderr)
        ...
        # 3. 寫入資料庫
        if hasattr(self, 'write_to_db'):
            try:
                self.write_to_db()
                print(f"  ✅ 資料庫寫入成功", file=sys.stderr)
            except Exception as e:
                print(f"⚠️ 資料庫寫入失敗: {e}", file=sys.stderr)
    ...
    def _print_final_statistics(self):
        ...
        if hasattr(self, 'no_dryer_records') and self.no_dryer_records:
            count = len(self.no_dryer_records)
            print(f"  凍乾機為空: {count} 筆", file=sys.stderr)
            total_records += count
        
        if hasattr(self, 'p0_strict_records') and self.p0_strict_records:
            count = len(self.p0_strict_records)
            print(f"  P0 (非實驗): {count} 筆", file=sys.stderr)
            total_records += count
        
        if hasattr(self, 'p0_experiment_records') and self.p0_experiment_records:
            count = len(self.p0_experiment_records)
            print(f"  P0 (實驗): {count} 筆", file=sys.stderr)
            total_records += count
        
        if hasattr(self, 'schedule_df') and not self.schedule_df.empty:
            count = len(self.schedule_df)
            print(f"  P1-P3 成功: {count} 筆", file=sys.stderr)
            total_records += count
            
            if '日期' in self.schedule_df.columns:
                days = self.schedule_df['日期'].nunique()
                print(f"    • 涵蓋天數: {days} 天", file=sys.stderr)
            
            if '凍乾機台' in self.schedule_df.columns:
                dryers = self.schedule_df['凍乾機台'].nunique()
                print(f"    • 使用凍乾機: {dryers} 台", file=sys.stderr)
        
        print(f"\n  總記錄數: {total_records} 筆", file=sys.stderr)
        print(f"{'='*70}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        print(f"📝 Log 记录已启动: {log_path}", file=sys.stderr)
        
    except Exception as e:
        print(f"⚠️ 无法建立 Log 档案: {e}")

    print(f"===== [{CURRENT_VERSION}] 排程开始 (P0-P3 模式) =====\n", file=sys.stderr)
    ...
    try:
        ...
        print(f"排程起始日期（周一）: {start_date.strftime('%Y-%m-%d')}\n", file=sys.stderr)
        
    except Exception as e:
        output = {"ok": False, "message": f"日期解析失败: {e}"}
    ...
    if args.batch_numbers:
        print("="*70, file=sys.stderr)
        print("🔢 设置工单批次编号", file=sys.stderr)
        print("="*70, file=sys.stderr)
    ...
    try:
        ...
        if not all_data or all_data["demand"] is None:
            output = {"ok": False, "message": "资料载入错误"}
            print(json.dumps(output, ensure_ascii=False))
            sys.exit(1)
        ...
        # ========================================
        print("\n" + "="*60, file=sys.stderr)
        print("🔮 产能预测模拟 (P0~P3 最少工期计算)", file=sys.stderr)
        ...
        print(f"\n💡 系统建议：", file=sys.stderr)
        print(f"   考量冻干机数量与 03-08 禁止收药限制，", file=sys.stderr)
        print(f"   理论最少需要工期: 【{min_required_days} 天】", file=sys.stderr)
        ...
        if min_required_days <= MAX_ALLOWED_DAYS:
            ...
            print(f"✅ 设定排程天数为 {scheduler.days_to_schedule} 天", file=sys.stderr)
        else:
            scheduler.days_to_schedule = MAX_ALLOWED_DAYS
            print(f"⚠️ 模拟天数超过上限，强制设定为 {MAX_ALLOWED_DAYS} 天", file=sys.stderr)
        ...
        if hasattr(scheduler, 'schedule_df') and not scheduler.schedule_df.empty:
            ...
            output = {"ok": False, "message": "排程已终止，未产生结果"}
            print(json.dumps(output, ensure_ascii=False))
            sys.exit(1)

    except Exception as e:
        output = {"ok": False, "message": f"排程失败: {str(e)}"}
        print(json.dumps(output, ensure_ascii=False))
        traceback.print_exc(file=sys.stderr)
        sys.exit(1)
