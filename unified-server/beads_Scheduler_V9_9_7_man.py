...
def is_person_on_vacation(person, date_obj):
    ...
    if date_str in VACATION_STAFF:
        return person in VACATION_STAFF[date_str]
    
    return False

def save_schedule_to_database(schedule_data_list, schedule_start_date):
    """
    將排程結果存入 SQLite 資料庫
    
    Args:
        schedule_data_list (list): 排程資料列表，每個元素為 dict
        schedule_start_date (datetime): 排程起始日期（週一）
        
    Returns:
        None
        
    資料表命名規則：
        schedule_YYYY (例如: schedule_2025, schedule_2026)
    """
    print("\n" + "="*70, file=sys.stderr)
    print(f"💾 Step 5: 儲存排程到資料庫", file=sys.stderr)
    print("="*70, file=sys.stderr)
    
    # 檢查是否有資料
    if not schedule_data_list:
        print("  ⚠️ 無資料需要儲存", file=sys.stderr)
        return
    ...
    try:
        ...
        # 1. 判斷資料表名稱（基於年份）
        # ========================================
        schedule_year = schedule_start_date.year
        
        # 檢查是否跨年（週六在下一年）
        saturday = schedule_start_date + timedelta(days=5)
        
        if saturday.year > schedule_year:
            table_year = saturday.year
            print(f"  ℹ️ 排程跨年：週一 {schedule_start_date.strftime('%Y-%m-%d')}, 週六 {saturday.strftime('%Y-%m-%d')}", file=sys.stderr)
            print(f"  ℹ️ 使用 {table_year} 年度表", file=sys.stderr)
        else:
            table_year = schedule_year
        
        # 修正：資料表名稱改為 schedule_YYYY 格式（SQLite 不允許數字開頭）
        table_name = f"schedule_{table_year}"
        
        # ========================================
        # 2. 轉換為 DataFrame
        # ========================================
        df_to_save = pd.DataFrame(schedule_data_list)
        ...
        # 計算排程週別（ISO 週曆）
        iso_year, iso_week, iso_day = schedule_start_date.isocalendar()
        df_to_save['schedule_week'] = f"{iso_year}-W{iso_week:02d}"
        ...
        with sqlite3.connect(SCHEDULE_DB_PATH) as conn:
            ...
            print(f"  ✅ 資料表 '{table_name}' 確認完成", file=sys.stderr)
            ...
            df_to_save.to_sql(
                table_name, 
                conn, 
                if_exists='append',  # 追加模式（不覆蓋現有資料）
                index=False          # 不寫入 DataFrame 的 index
            )
            
            # ========================================
            # 6. 查詢統計資訊
            # ========================================
            schedule_week = df_to_save['schedule_week'].iloc[0]
            
            # 本次寫入筆數
            records_written = len(df_to_save)
            
            # 本週總計筆數
            cursor = conn.execute(
                f"SELECT COUNT(*) FROM {table_name} WHERE schedule_week = ?", 
                (schedule_week,)
            )
            ...
            # 7. 顯示結果
            # ========================================
            print(f"  ✅ 資料已儲存至: {SCHEDULE_DB_PATH}", file=sys.stderr)
            print(f"  📋 表格名稱: {table_name}", file=sys.stderr)
            print(f"  📊 本次寫入: {records_written} 筆", file=sys.stderr)
            print(f"  📊 本週 ({schedule_week}) 總計: {total_this_week} 筆", file=sys.stderr)
            print(f"  📊 資料表總計: {total_all} 筆", file=sys.stderr)
            
            if type_stats:
                print(f"\n  📈 本週排程類型分佈:", file=sys.stderr)
                for record_type, count in type_stats:
                    type_name_map = {
                        'P1-P5': 'P1-P5 正常排程',
                        'P0_strict': 'P0 非實驗（深黃色）',
                        'P0_experiment': 'P0 實驗',
                        'no_dryer': '凍乾機為空'
                    }
                    type_name = type_name_map.get(record_type, record_type)
                    print(f"      • {type_name}: {count} 筆", file=sys.stderr)
            
    except Exception as e:
        print(f"  ❌ 資料庫儲存失敗: {e}", file=sys.stderr)
        print(f"  錯誤詳情:", file=sys.stderr)
...
def load_all_data(demand_xlsx_path):
    ...
    try:
        ...
        df_demand = df_demand.rename(columns={
            col_pn: "料號", col_stock: "Stock_plus_Dosing",
            col_w1: "第一周需求", col_w2: "第二周需求", col_w3: "第三周需求"
        })
        df_demand = df_demand[["料號", "Stock_plus_Dosing", "第一周需求", "第二周需求", "第三周需求"]]
    ...
    df_merged = pd.merge(df_demand, df_bdc, on="料號", how="left")
    ...
    try:
        ...
        db_tables["Beads_Dry_Count_Info"] = df_bdc.set_index("料號")
        db_tables["Dosing_Constraints"] = df_constraints.set_index("PN")

        # === V9.9: 載入 P0 (限制 OR 插單) ===
        try:
            df_p0_orders = pd.read_sql("SELECT * FROM '限制 OR 插單'", conn)
...
def calculate_demand_queue(data: dict):
    ...
    for _, r in df.iterrows():
        ...
        if w1_short > 0:
            if pn in w1_urgent_wed_pn: plan_prio = 0
            elif pn in w1_urgent_other_pn: plan_prio = 1
            else: plan_prio = 2
            subp = (dryer_prio, person_prio, plan_prio)
            task_queue.append((1, subp, pn, w1_short, "W1", group_name, marker_name, prod_qty))
        
        if w2_short > 0:
            subp = (dryer_prio, person_prio, 0)
            task_queue.append((2, subp, pn, w2_short, "W2", group_name, marker_name, prod_qty))
        
        if w3_short > 0:
            subp = (dryer_prio, person_prio, 0)
            task_queue.append((3, subp, pn, w3_short, "W3", group_name, marker_name, prod_qty))
    ...
    if idle_pns:
        shortage_pns = set(t[2] for t in task_queue)
        df_all_demand_data = data["demand"]
        
        df_idle_tasks_data = df_all_demand_data[
            df_all_demand_data['料號'].isin(idle_pns) &
            ~df_all_demand_data['料號'].isin(shortage_pns)
        ]
    ...
    task_queue.sort(key=lambda x: (x[0], x[1][0], x[1][1], x[1][2])) 
    task_dict = {t[2]: t for t in task_queue}

    print(f"✅ 需求計算完成: {len(task_queue)} 個任務\n", file=sys.stderr)
...
class ResourceManager:
    ...
    def book_p0_strict_person(self, person, shift_key, day_key):
        ...
        if person in self.p0_strict_person_usage[shift_key]:
            print(f"    ⚠️ P0 警告: 人員 {person} 在 {shift_key} 已被其他 P0 任務佔用", file=sys.stderr)
        
        if person in self.person_usage[shift_key]:
            print(f"    ⚠️ P0 警告: 人員 {person} 在 {shift_key} 已被 P1-P5 任務佔用", file=sys.stderr)

        self.p0_strict_person_usage[shift_key].add(person)
        self.person_usage[shift_key].add(person)
        print(f"    🟢 P0 預訂人員: {person} ({shift_key})", file=sys.stderr)
    ...
    def book_p0_experiment_person(self, person, shift_key):
        ...
        if person in self.p0_experiment_person_usage[shift_key]:
            print(f"    ⚠️ P0 (實驗) 警告: 人員 {person} 在 {shift_key} 已被其他實驗 P0 佔用", file=sys.stderr)
        
        self.p0_experiment_person_usage[shift_key].add(person)
        print(f"    🧪 P0 (實驗) 預訂人員: {person} ({shift_key})", file=sys.stderr)
    ...
    def can_use_person(self, person, shift_key, day_key, is_single_slot, priority=None):
        ...
        try:
            ...
            if is_person_on_vacation(person, date_obj):
                return False, f"{person} 在 {day_key} 請假"
        except Exception as e:
            print(f"              ⚠️ 無法檢查 {person} 的請假狀態: {e}", file=sys.stderr)
        
        # 檢查非實驗 P0 衝突（所有 P1-P5 都不能衝突）
        if person in self.p0_strict_person_usage.get(shift_key, set()):
            return False, f"{person} 被 P0 (非實驗) 佔用"
        
        # 檢查實驗 P0 衝突（僅 P3-P5 不能衝突）
        if priority is not None and priority >= 3:
            if person in self.p0_experiment_person_usage.get(shift_key, set()):
                return False, f"{person} 被 P0 (實驗) 佔用"

        # 檢查 P1-P5 衝突
        if person in self.person_usage.get(shift_key, set()):
            return False, f"{person} 在 {shift_key} 已被佔用"
        
        # 檢查單次限制
        if is_single_slot and person in self.person_day_limit.get(day_key, set()):
    ...
    def book_person(self, person, shift_key, day_key, is_single_slot, priority=None):
        ...
        if is_single_slot:
            if day_key not in self.person_day_limit:
                self.person_day_limit[day_key] = set()
            self.person_day_limit[day_key].add(person)
        
        print(f"        🟢 預訂人員: {person} ({shift_key})", file=sys.stderr)
...
class Scheduler:
    ...
    def _create_dummy_task(self, pn):
        try:
            bdc_info = self.beads_dry_info.loc[pn]
            group_name = bdc_info["BDC_Group_Name"]
    ...
    def find_batch(self, current_task_info):
        ...
        for pn_in_group in pns_in_group:
            ...
            if pn_in_group in self.beads_dry_info.index:
                bdc_data = self.beads_dry_info.loc[pn_in_group]
                if isinstance(bdc_data, pd.DataFrame):
                    prod_qty = bdc_data.iloc[0]["BDC_Prod_Qty"]
                else:
                    prod_qty = bdc_data["BDC_Prod_Qty"]
            ...
            if pn_in_group in self.task_dict:
                batch_tasks.append(self.task_dict[pn_in_group])
            else:
                dummy = self._create_dummy_task(pn_in_group)
                if dummy:
    ...
    def _calculate_ports_and_time(self, prod_qty):
        """根據新的 Port 規則"""
        min_ports_default = min(PORT_OPTIONS) if PORT_OPTIONS else 2
        
        if prod_qty == 0:
            return min_ports_default, 0.5 

        for p in PORT_OPTIONS:
            time_hrs = prod_qty / (DOSING_RATE_PER_HR * p)
            if DOSING_TIME_MIN <= time_hrs <= DOSING_TIME_MAX:
                return p, time_hrs 
                
        time_at_max_p = prod_qty / (DOSING_RATE_PER_HR * MAX_PORTS_PER_PN)
        if time_at_max_p > DOSING_TIME_MAX:
            return MAX_PORTS_PER_PN, time_at_max_p

        min_ports = min_ports_default
        return min_ports, prod_qty / (DOSING_RATE_PER_HR * min_ports)
        ...
        for c in constraints_list:
            for key in ["配藥人-1", "配藥人-2", "配藥人-3"]:
                ...
                if person and str(person).strip():
                    all_people.add(str(person).strip())
    ...
    def find_available_dryer_v10(self, constraints_list, delivery_dt, priority=None):
        """V10: 簡化的凍乾機查找"""
        day_key = delivery_dt.strftime('%Y-%m-%d')
    ...
    def check_availability(self, batch_tasks, constraints_list, delivery_dt, slot_name, priority=None):
        """檢查資源可用性"""
        day_key = delivery_dt.strftime('%Y-%m-%d')
        is_ivek_task = any(pn in IVEK_COMPONENTS for pn in [t[2] for t in batch_tasks])
        ...
        dryer_id = self.find_available_dryer_v10(constraints_list, delivery_dt, priority)
        if not dryer_id:
            return None
        ...
        try:
            ...
            num_ports, dosing_hrs = self._calculate_ports_and_time(max_qty)
    ...
    def book_resources(self, resources, batch_tasks, delivery_dt, slot_name, priority=None):
    ...
    def handle_p0_orders(self):
        """
        V9.9.2: 處理 P0 (插單/限制) 任務（支援休假日檢查）
        規則1: 必須有日期和 Marker
        規則2: 滴定機或凍乾機為空 → no_dryer
        規則3: 備註含「*實驗*」→ p0_experiment_records (P2 後排)
        規則4: 備註無「*實驗*」→ p0_strict_records (優先排)
        規則5: 若 P0 日期為休假日 → 發出警示並終止排程
        """
        print("\n" + "="*70, file=sys.stderr)
        ...
        for _, row in df_p0_orders.iterrows():
            # === 規則1: 驗證日期和 Marker ===
            ...
            if not date_str or not marker:
                print(f"❌ P0 任務 {lot} 失敗: 缺少日期或 Marker. 跳過.", file=sys.stderr)
                continue
            ...
            try:
                ...
                match_md = re.search(r'(\d{1,2})月(\d{1,2})日', date_str_clean)
                ...
                if match_md:
                    ...
                    task_date = pd.to_datetime(date_str_clean)
                
                # ========================================
                # V9.9.2: 檢查 P0 日期是否為休假日
                # ========================================
                if is_holiday(task_date):
                    ...
                    print(f"    ⚠️ P0 任務 {lot} 的日期 {task_date.strftime('%Y-%m-%d')} 為休假日", file=sys.stderr)
                
                if monday <= task_date <= saturday:
                    p0_tasks_this_week.append((task_date, row))
                        
            except Exception as e:
                print(f"    ❌ P0 日期解析失敗: '{date_str}' - {e}. 跳過.", file=sys.stderr)
        
        # ========================================
        # V9.9.2: 若有 P0 落在休假日，立即終止排程
        # ========================================
        if p0_on_holiday:
            print("\n" + "="*70, file=sys.stderr)
            print("❌ 排程終止：P0 任務日期衝突", file=sys.stderr)
            print("="*70, file=sys.stderr)
            print(f"以下 {len(p0_on_holiday)} 個 P0 任務的排程日期為休假日，無法排程:", file=sys.stderr)
            for p0_info in p0_on_holiday:
                print(f"  • {p0_info['lot']} ({p0_info['date']})", file=sys.stderr)
            
            print("\n請調整以下任一項目後重新排程:", file=sys.stderr)
            print("  1. 修改 P0 任務的日期（在 '限制 OR 插單' 表中）", file=sys.stderr)
            print("  2. 移除相應的休假日設定", file=sys.stderr)
            print("  3. 取消這些 P0 任務", file=sys.stderr)
            print("="*70, file=sys.stderr)
        ...
        if not p0_tasks_this_week:
            print("  ... 無本週 P0 任務", file=sys.stderr)
            return

        print(f"  ... 找到 {len(p0_tasks_this_week)} 筆本週 P0 任務（均非休假日）", file=sys.stderr)
        ...
        for task_date, row in p0_tasks_this_week:
            ...
            if not staff:
                print(f"❌ P0 任務 {lot} ({day_key}) 失敗: 未指定配藥同仁. 跳過.", file=sys.stderr)
                continue
            # ========================================
            # V9.9.9: [新增] 檢查 P0 配藥人員是否請假
            # ========================================
            if is_person_on_vacation(staff, task_date):
                print(f"❌ P0 任務 {lot} ({day_key}) 失敗: 配藥人員 {staff} 請假", file=sys.stderr)
                print(f"    請調整 P0 任務的配藥人員或休假人員設定", file=sys.stderr)
                continue
            try:
            ...
            print(f"  🅿️ 處理 P0: {lot} (日期: {day_key} {slot_name}, 人員: {staff}, 備註: '{remark}')", file=sys.stderr)

            # === 規則2: 滴定機或凍乾機為空 → no_dryer ===
            if pd.isna(dryer) or dryer == "" or pd.isna(machine) or machine == "":
                print(f"    ... P0 任務 {lot} 無滴定機或凍乾機，轉 no_dryer (PM Port12 下方)...", file=sys.stderr)
                ...
                if person_result is None:
                    print(f"    ❌ P0 任務 {lot} 失敗: 無法為 {staff} 預訂全天資源. 跳過.", file=sys.stderr)
                    continue
                ...
                print(f"    ✅ P0 (no_dryer) 任務 {lot} 已排入 {day_key}", file=sys.stderr)
                continue
            ...
            if is_experiment:
                # 規則3: 實驗 P0 → P2 後排，允許衝突
                print(f"    📋 P0 ({lot}) 為實驗任務，將在 P2 後排程（允許與 P1/P2 衝突）", file=sys.stderr)
                ...
                # 規則4: 非實驗 P0 → 優先排，強制預訂資源
                print(f"    🔒 P0 ({lot}) 為非實驗任務，優先排程（P1-P5 不能衝突）", file=sys.stderr)
                ...
                try:
                    ...
                    if "ivek" in machine:
                        ...
                        try:
                            ...
                            print(f"    ⚠️ P0 警告: 無法解析滴定機 '{machine}': {e}", file=sys.stderr)
                    ...
                    self.scheduled_pns.add(lot)
                    print(f"    ✅ P0 (非實驗) 任務 {lot} 已強制排入 {shift_key}", file=sys.stderr)
                    
                except Exception as e:
                    print(f"  ❌ P0 任務 {lot} 發生錯誤: {e}. 跳過.", file=sys.stderr)
    ...
    def run(self):
        ...
        task_priorities.sort(key=lambda x: (x[0], x[1][0], x[1][1], x[1][2], x[2]))
                ...
                for slot_name in ["AM", "PM"]:
                    ...
                    if slot_name == "AM":
                        ...
                        if not day_batch_times[day_key]["AM"]:
                            ...
                            # 檢查是否超過 AM 結束時間（15:00）
                            if next_time >= time(15, 0):
                                if not fail_reason:
                                    fail_reason = f"{day_key} AM 時段已滿"
                                continue
                    else:
                        # PM 班次：從 15:30 開始
                        if not day_batch_times[day_key]["PM"]:
                            ...
                            # 檢查是否超過 PM 結束時間（17:30）
                            if next_time >= PM_END:
                                if not fail_reason:
                                    fail_reason = f"{day_key} PM 時段已滿"
                                continue
                    ...
                    if final_len > initial_len:
                        # 成功排程
                        self.book_resources(resources, batch_tasks, delivery_dt, slot_name, priority=week)
                        ...
                        if week != 5:
                            self.scheduled_pns.update(batch_pns)
        ...
        # ========================================
        # V9.9.2: 檢查 P1 任務是否全部完成
        # ========================================
        if p1_total > 0 and p1_scheduled < p1_total:
            print("\n" + "="*70, file=sys.stderr)
            print("❌ 排程終止：P1 任務未全部滿足", file=sys.stderr)
            print("="*70, file=sys.stderr)
            print(f"P1 任務統計:", file=sys.stderr)
            print(f"  總計: {p1_total} 個任務", file=sys.stderr)
            print(f"  已排程: {p1_scheduled} 個任務", file=sys.stderr)
            print(f"  未排程: {p1_total - p1_scheduled} 個任務", file=sys.stderr)
            print(f"\n未排程的 P1 任務:", file=sys.stderr)
            ...
            for week, subp, person_count, task in task_priorities:
                if week == 1:
                    ...
                    if pn not in self.scheduled_pns:
                        ...
                        print(f"  • {batch_name} (PN: {pn}) - 失敗原因: {reason}", file=sys.stderr)
        ...
        print(f"    - P1: {p1_scheduled}/{p1_total} ✅", file=sys.stderr)
    ...
    def book_and_record(self, resources, batch_tasks, current_day, slot_name, priority=None):
        ...
        batch_name = batch_tasks[0][5]
        print(f"    → {batch_name}: {len(rows)} PN, Port {port_start}-{current_port_idx-1}", file=sys.stderr)
    ...
    """
    V9.9.3: 修改後的 save_records_to_excel 方法
    此方法會將排程結果同時輸出到 Excel 和 SQLite 資料庫
    """

    def save_records_to_excel(self, outdir): # ✅ 修正 1: 增加 outdir 參數
        """
        V9.9.3: 將排程結果輸出到 Excel 並存入資料庫
        - P0 (非實驗): 深黃色
        - P0 (實驗，無衝突): 標準格式
        - P0 (實驗，有衝突): 淺紅色
        - P1-P5: 標準格式（拆分為單 Port 行，含工單號碼和 Batch）
        - 凍乾機為空: 放在 PM Port12 下方
        - 新增：將所有資料存入 SQLite 資料庫
        """
        print("\n" + "="*70, file=sys.stderr)
        print(f"📊 Step 4: 輸出 Excel ({CURRENT_VERSION})", file=sys.stderr)
        print("="*70, file=sys.stderr)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # ✅ 修正 2: 使用傳入的 outdir 參數，而不是全域變數 output_dir
        ...
        # 準備樣式
        p0_strict_fill = PatternFill(start_color="FFD700", end_color="FFD700", fill_type="solid")
        p0_conflict_fill = PatternFill(start_color="FFB6C1", end_color="FFB6C1", fill_type="solid")
        ...
        print(f"\n  📊 使用批次編號: {batch_start}", file=sys.stderr)
        
        # ========================================
        # 1. 輸出 P1-P5 正常排程（新格式）
        # ========================================
        if not self.schedule_df.empty:
            print(f"  寫入 P1-P5 排程: {len(self.schedule_df)} 筆", file=sys.stderr)
            
            # 按日期和班次排序
            df_sorted = self.schedule_df.sort_values(by=["日期", "班次", "滴定機"])
            
            # 工單號碼計數器（每個 PN 一個工單號碼）
            work_order_counter = batch_start
            pn_work_order_map = {}  # PN -> 工單號碼
            
            # Batch 計數器（每個 PN 每週的 Port 計數）
            pn_week_port_counter = {}  # (PN, Week) -> 計數
            ...
            for _, row in df_sorted.iterrows():
                ...
                if not ports:
                    print(f"  ⚠️ 警告: {pn} 無有效 Port 資訊，跳過", file=sys.stderr)
                    continue
                ...
                # 生成工單號碼（相同 PN 使用相同工單號碼）
                if pn not in pn_work_order_map:
                    try:
                        schedule_date = datetime.strptime(date_str, "%Y-%m-%d")
                        year_yy = schedule_date.strftime("%y")  # 25
                        month_char = chr(64 + schedule_date.month)  # 11月 = K (65=A, 所以64+11=75=K)
                ...
                try:
                    ...
                    print(f"  ⚠️ 日期解析錯誤: {date_str}, 使用預設值", file=sys.stderr)
                    iso_week = 1
                    year_yy = "25"
                    pn_last3 = "000"
                ...
                for port_num in ports:
                    ...
                    if port_count <= 8:
                        ...
                        # 10=B, 11=C, ..., 33=X
                        count_char = chr(65 + (port_count - 9))  # 65=A
                    ...
                    # V9.9.3: 加入資料庫儲存列表
                    # ========================================
                    schedule_data_for_db.append(row_data.copy())
        ...
        if self.p0_strict_records:
            print(f"  寫入 P0 (非實驗): {len(self.p0_strict_records)} 筆", file=sys.stderr)
            
            # P0 使用舊格式（包含班次，不含工單號碼和 Batch）
            p0_headers = [
                "日期", "marker", "滴定機", "凍乾機台", "配藥同仁",
                "RD給藥時間", "預計滴定時間", "預計結束", "預冷時間",
                "凍乾時間", "收藥時間", "數量", "lot", "", "班次"
            ...
            for p0_data in self.p0_strict_records:
                ...
                # 寫入 Excel
                for col_idx, header in enumerate(p0_headers, 1):
                    value = row_data.get(header, "")
                    cell = ws.cell(row=current_row, column=col_idx, value=value)
                    cell.alignment = standard_alignment
                    cell.border = standard_border
                    cell.fill = p0_strict_fill
        ...
        # ========================================
        if self.p0_experiment_records:
            print(f"  寫入 P0 (實驗): {len(self.p0_experiment_records)} 筆", file=sys.stderr)
        ...
        if self.no_dryer_records:
            print(f"  寫入凍乾機為空任務: {len(self.no_dryer_records)} 筆", file=sys.stderr)
            ...
            sorted_no_dryer = sorted(self.no_dryer_records, key=lambda x: x["日期"])
            
            for record in sorted_no_dryer:
                row_data = {
                    "日期": record.get("日期", ""),
        ...
        try:
            ...
            print(f"  總筆數: {current_row - 2}", file=sys.stderr)
            print(f"  P1-P5 (拆分後): {current_row - 2 - len(self.p0_strict_records) - len(self.p0_experiment_records) - len(self.no_dryer_records)}", file=sys.stderr)
            print(f"  P0 (非實驗/深黃色): {len(self.p0_strict_records)}", file=sys.stderr)
            print(f"  P0 (實驗): {len(self.p0_experiment_records)}", file=sys.stderr)
            print(f"    - 有衝突 (淺紅色): {sum(1 for p in self.p0_experiment_records if p.get('has_conflict', False))}", file=sys.stderr)
            print(f"    - 無衝突: {sum(1 for p in self.p0_experiment_records if not p.get('has_conflict', False))}", file=sys.stderr)
            print(f"  凍乾機為空: {len(self.no_dryer_records)}", file=sys.stderr)
        ...
        if schedule_data_for_db:
            ...
            if SCHEDULE_START_DATE is None:
                print("\n⚠️ 警告: 無法取得排程起始日期，跳過資料庫儲存", file=sys.stderr)
            else:
                save_schedule_to_database(schedule_data_for_db, SCHEDULE_START_DATE)
        else:
            print("\n⚠️ 無排程資料需要儲存到資料庫", file=sys.stderr)
    ...
    try:
        ...
        if hasattr(scheduler, 'schedule_df') and not scheduler.schedule_df.empty:
            ...
            if args.dry_run:
                ...
                # ❌ 移除：以下兩行是多餘的，因為 output_path 已經從函數返回
                # timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                # output_path = os.path.join(args.outdir, f"排程結果_{timestamp}.xlsx")
