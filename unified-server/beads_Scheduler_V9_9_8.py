...
class Scheduler:
    ...
    def save_records_to_excel(self, outdir):
        ...
        # 🔥 新的排序邏輯（固定區塊）
        # ========================================
        def sort_records_fixed_blocks(rec):
            """
            排序為固定區塊：
            區塊 0: IVEK
            區塊 1: AM Port1-12
            ...
            # 其他（例如 P0 的特殊機器名稱）
                ...
                # 嘗試判斷是否包含 Port 數字
                port_match = re.search(r'[Pp]ort\s*(\d+)', machine)
                if port_match:
        ...
        for date_key in sorted(records_by_date.keys()):
            ...
            # 🔥 使用固定區塊排序
            records.sort(key=sort_records_fixed_blocks)
            ...
            for rec in records:
                ...
                if "IVEK" in machine.upper():
                    ...
                    current_block = "no_dryer"
                elif "Port" in machine:
                    current_block = "AM_Port" if slot == "AM" else "PM_Port"
                else:
                    current_block = "other"
                ...
                if last_block is not None and current_block != last_block:
                    # 加入空行
                    current_row += 1
                    print(f"      → 區塊切換: {last_block} → {current_block}，加入空行於 row {current_row-1}", file=sys.stderr)
                ...
                db_record = {
                    ...
                    "凍乾機台": rec.get("凍乾機台", ""),
                    "配藥同仁": rec.get("配藥同仁", ""),
                    "RD給藥時間": rec.get("RD給藥時間", ""),
                    "預計滴定時間": rec.get("預計滴定時間", ""),
                    "預計結束": rec.get("預計結束", ""),
                    "預冷時間": "",
                    "凍乾時間": "",
                    "收藥時間": "",
            ...
            print(f"  ✅ 工作表 '{sheet_name}' 已建立 ({len(records)} 筆資料)", file=sys.stderr)
