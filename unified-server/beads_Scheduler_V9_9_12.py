...
def calculate_demand_queue(data: dict):
    ...
    # ==========================================================
    # ✅ 修正：按 BDC_Group_Name (藥名) 分組計算總需求
    # ==========================================================
    ...
    # 分離 tCREA 和 其他
    df_tcrea = df_clean[df_clean['BDC_Group_Name'] == 'tCREA']
    df_others = df_clean[df_clean['BDC_Group_Name'] != 'tCREA']
    
    df_grouped_list = []

    # 處理 tCREA
    if not df_tcrea.empty:
        print("  ... 偵測到 tCREA 特例，執行 1:1:2 標準化...", file=sys.stderr)
        df_tcrea_norm = df_tcrea.copy()
        
        # 找出 2x (48000) 的那一行
        mask_2x = (df_tcrea_norm['料號'] == '5714400182') | (df_tcrea_norm['BDC_Prod_Qty'] == 48000.0)
        
        # 定義要標準化 (除以 2) 的欄位
        cols_to_divide = ['Stock_plus_Dosing', '第一周需求', '第二周需求', '第三周需求', 'BDC_Prod_Qty']
        
        # 執行標準化 (除以 2)
        df_tcrea_norm.loc[mask_2x, cols_to_divide] = df_tcrea_norm.loc[mask_2x, cols_to_divide].div(2)
        
        # 標準化後, tCREA 也可以用 1:1 的 min/max 邏輯來聚合
        df_tcrea_agg = df_tcrea_norm.groupby('BDC_Group_Name').agg(agg_cols).reset_index()
        df_grouped_list.append(df_tcrea_agg)

    # 處理其他所有藥品 (使用標準 1:1 邏輯)
    if not df_others.empty:
        df_others_grouped = df_others.groupby('BDC_Group_Name').agg(agg_cols).reset_index()
        df_grouped_list.append(df_others_grouped)

    # 合併 tCREA 和 其他藥品
    if not df_grouped_list:
        print("  ⚠️ 警告: 需求計算分組後為空。", file=sys.stderr)
        return [], {}
        
    df_grouped = pd.concat(df_grouped_list, ignore_index=True)

    # ==========================================================
    # 4. 遍歷分組後的 df_grouped (而不是原本的 df)
    # ==========================================================
    task_queue = []
    ...
    for _, r in df_grouped.iterrows():
        ...
        # 取得這個群組包含的所有 PN (用於 P1 檢查)
        pns_in_group = r.get("料號", [])

        # ✅ 修正：必須同時檢查 0 和 NaN (NaN from bad merge)
        ...
        # 檢查 P1 半成品優先級 (檢查群組中任一 PN 是否在 P1 清單)
        ...
        # 任務的 "PN" 欄位 (t[2]) 
        # 我們選擇 pns_in_group 中的第一個 PN 作為代表
        # `find_batch` 函數會根據 group_name 重新找出所有 PN，所以這裡選哪個不重要
    ...
    if idle_pns:
        # P4/P5 仍然基於 df (原始需求表), 而不是 df_grouped
        # 因為 P4/P5 邏輯是檢查 "哪些PN被標記"
        shortage_pns_in_groups = set(t[5] for t in task_queue) # 取得有 P1-P3 任務的 "群組"
        df_all_demand_data = data["demand"]
        
        df_idle_tasks_data = df_all_demand_data[
            df_all_demand_data['料號'].isin(idle_pns) &
            ~df_all_demand_data['BDC_Group_Name'].isin(shortage_pns_in_groups)
        ]
        ...
        for _, r in df_idle_tasks_data.iterrows():
            group_name = r["BDC_Group_Name"]
            if group_name in added_groups_p4:
                continue
        ...
        for _, r in df_idle_tasks_data.iterrows():
            group_name = r["BDC_Group_Name"]
            if group_name in added_groups_p5:
                continue
