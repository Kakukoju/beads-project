...
class Scheduler:
    ...
    def run(self):
        """
        ✅ 修改版: 只排 P0-P3 核心任務，使用最少天數
        """
        import sys
        from datetime import timedelta, datetime
        
        print(f"\nStep 3: 開始排程 (只排 P0-P3 核心任務)...\n", file=sys.stderr)
        ...
        for day_index in range(self.days_to_schedule):
            ...
            if is_holiday(current_day):
                print(f"  🗓️ 跳過休假日: {current_day.strftime('%Y-%m-%d')}", file=sys.stderr)
                continue
        ...
        print(f"生成 {self.days_to_schedule} 個排程日\n", file=sys.stderr)
        ...
        # ========================================
        print("="*70, file=sys.stderr)
        print("📋 Step 3.2: 開始核心任務排程 (P1-P3)", file=sys.stderr)
        print("="*70, file=sys.stderr)
        ...
        task_priorities.sort(key=lambda x: (
            ...
            0 if x[3][2] in self.RULE_GLIPA_PNs else 1,      # 2. GLIPA VIP (修正這裡)
            x[1][0],                                          # 3. 機台數
            x[2],                                             # 4. 人數
            x[3][5],                                          # 5. 藥名
            x[3][2]                                           # 6. PN
            ...
            for day_offset in range(self.days_to_schedule):
                ...
                for slot_name in ["AM", "PM"]:
                    ...
                    if next_time is None:
                        if not fail_reason: fail_reason = f"{day_key} {slot_name} 時段已滿"
                        continue
                    ...
                    if is_tcrea_batch1:
                        ...
                        if not can_schedule_batch2:
                            print(f"  ⚠️ tCREA 同日綁定失敗: 跳過 {day_key}", file=sys.stderr)
                    ...
                    if is_blocking_batch:
                        ...
                        print(f"  ✅ 記錄凍乾機 {dryer_id} 封鎖至 {blocked_until_date}", file=sys.stderr)
        ...
        print(f"  P1-P3 成功: {scheduled_count} 批次", file=sys.stderr)
        print(f"    - P1: {p1_scheduled_count}/{p1_total} {'✅' if p1_scheduled_count == p1_total else '❌'}", file=sys.stderr)
        
        unmet_pns = [t[2] for t in self.task_queue if t[2] not in self.scheduled_pns]
        print(f"  未排入 PN (P1-P3): {len(unmet_pns)}", file=sys.stderr)
        
        print(f"\n✅ 核心任務排程完成", file=sys.stderr)
