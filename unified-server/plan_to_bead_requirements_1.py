...
def get_bead_inventory_totals(db_path: str) -> Dict[str, int]:
    ...
    規則：
      ...
      2) 「可使用庫存」< 300 的列不計算（但 PN 在 allow_small_pns 例外名單就仍計算, 10/7 edit <500 ask from yocheng）
    ...
    for r in rows:
        ...
        # 規則 2
        if candidate < 500 and pn not in allow_small_pns:
            continue
...
def write_back_to_sheet(xlsx_path: str,
                        ...
                        inv_by_pn: dict[str, int]) -> str:        # ← 回傳路徑
    """
    目標工作表：B 欄=半品料號
...
if __name__ == "__main__":
    ...
    parser.add_argument("--date", type=str, required=True, help="起始日期 mm/dd")
    parser.add_argument("--out", type=str,  required=True)
    parser.add_argument("--dry", type=int, default=0, help="1=dry run, 0=寫入")
    ...
    if args.dry:
        # Dry Run → 產生 JSON 給 UI 顯示
        rows = []
        wb = openpyxl.load_workbook(PANEL_XLSX, data_only=True)
        try:
            ws = wb[TARGET_SHEET]
            max_row = ws.max_row
            for r in range(3, max_row + 1):
                drug = ws.cell(r, 1).value  # 藥名
                pn   = _normalize_pn(ws.cell(r, 2).value)  # 料號
                name = ws.cell(r, 3).value  # 品名
                ly   = _to_number(ws.cell(r, 4).value)     # 凍乾數 (假設D欄)
                inv  = inv_by_pn.get(pn, 0)
                w1, w2, w3 = bead3w.get(pn, (0,0,0))
                rows.append({
                    "藥名": drug,
                    "料號": pn,
                    "品名": name,
        ...
        print(json.dumps({"ok": True, "data": rows}, ensure_ascii=False))
    else:
        # 寫回 Excel
        real_out = write_back_to_sheet(PANEL_XLSX, TARGET_SHEET, bead3w, inv_by_pn)
        print(json.dumps({"ok": True, "msg": "✅ 已寫入 Excel", "out_path": real_out}, ensure_ascii=False))
