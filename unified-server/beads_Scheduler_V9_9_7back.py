...
class Scheduler:
    ...
    def save_records_to_excel(self, outdir):
        """
        V9.9.4: 將排程結果輸出到 Excel（含空行分隔和多工作表）
        - 一天一個工作表
        - 不同批次之間加空行
        - 滴定機按 Port 順序排列
        - 移除顏色標記
            ...
            for _, row in df_sorted.iterrows():
                ...
                for port_num in ports:
                    ...
                    record = {
                        ...
                        "班次": row.get("班次", "AM"),
                        "record_type": "P1-P5",
                        "batch_group": row.get("marker", "")  # 用於分組
                    }
        ...
        if self.p0_strict_records:
            ...
            for p0_data in self.p0_strict_records:
                ...
                record = {
                    ...
                    "batch_group": str(p0_data.get("Marker", "")).strip()
                }
        ...
        if self.no_dryer_records:
            ...
            for no_dryer_rec in self.no_dryer_records:
                ...
                record = {
                    ...
                    "batch_group": no_dryer_rec.get("marker", "")
                }
        ...
        for date_key in sorted(records_by_date.keys()):
            ...
            def sort_records(rec):
                ...
                if "Port" in machine:
                    try:
                        machine_order = int(re.search(r'\d+', machine).group())
                    except:
                        pass
                elif "IVEK" in machine.upper():
                    machine_order = 0
                elif machine == "":
                    machine_order = 1000
                
                return (slot_order, machine_order)
            
            records.sort(key=sort_records)
            
            # 建立工作表
            sheet_name = date_key  # 2025-11-03
            ...
            for record in records:
                # 檢查是否需要加空行（不同批次）
                current_batch_group = record.get("batch_group", "")
                if last_batch_group and current_batch_group != last_batch_group:
                    # 加入空行
                    current_row += 1
                
                last_batch_group = current_batch_group
            ...
            print(f"  ✅ 工作表 '{sheet_name}' 已建立 ({len(records)} 筆)", file=sys.stderr)
        ...
        try:
            ...
            print(f"   共 {len(records_by_date)} 個工作表", file=sys.stderr)
        except Exception as e:
