...
====================================================================
功能：
1. Excel ↔ SQLite 同步服務 (A/B/C/D/E 五個檔案)
2. Flask Web API 服務
3. 排程管理與查詢
4. 需求統計模組
====================================================================
...
import sys
import os
# --- 強制加入當前目錄到系統路徑 (解決 Import 找不到的問題) ---
...
from IPQA_db_V1_importable import start_monitoring, DEFAULT_CONFIG as IPQC_CONFIG
# ====================================================================
# ====== 配置與常量 ======
# ====================================================================
...
# IPQC_DB_PATH = IPQC_CONFIG["DB_PATH"]
# main flask.py 裡面的設定
...
def initialize_sync_service():
    ...
    try:
        ...
        # 初始同步
        logger.info("步驟 1/3: 執行初始同步")
        initial_sync_and_prepare_db()

        # [NEW] 初始化 WIP 自動化
        logger.info("\n步驟 2/3: 初始化 WIP 自動化系統")
        try:
            init_wip_automation(app)
        except Exception as e:
            logger.error(f"❌ WIP 初始化異常: {e}")
        ...
        if state.observer and state.observer.is_alive():
            logger.info("=" * 60)
            logger.info("✅ 同步服務初始化成功")
            logger.info("=" * 60)
            state.sync_init_done = True
        else:
            logger.error("=" * 60)
            logger.error("❌ Watchdog 未成功啟動！")
            logger.error("=" * 60)
...
def _init_before_request():
    ...
    if not state.sync_init_done:
        logger.info("📍 before_request 觸發初始化")
...
def test_db():
    ...
    try:
        ...
        with sqlite3.connect(FORMULATE_DB_PATH) as conn:
            ...
            # 檢查 DropletSchedule 表結構
            table_info = {}
            if 'DropletSchedule' in tables:
                cursor.execute("PRAGMA table_info(DropletSchedule)")
                columns = cursor.fetchall()
                table_info['DropletSchedule'] = {
                ...
                # 取得資料筆數
                cursor.execute("SELECT COUNT(*) FROM DropletSchedule")
                table_info['DropletSchedule']['row_count'] = cursor.fetchone()[0]
                
                # 取得最近5筆資料
                cursor.execute("SELECT * FROM DropletSchedule LIMIT 5")
                table_info['DropletSchedule']['sample_data'] = cursor.fetchall()
            
            return jsonify({
                "ok": True,
                "database_path": FORMULATE_DB_PATH,
                "tables": tables,
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Flask/同步服務整合啟動（重構版）")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"📂 同步資料庫: {MAIN_DB_PATH}")
        logger.info(f"📂 排程資料庫: {DB_PATH}")
        logger.info(f"📄 A 檔: {FILE_A_CONFIG.path}")
        logger.info(f"📄 B 檔目錄: {get_plan_dir()}")
        logger.info(f"📄 C 檔: {FILE_C_CONFIG.path}")
        logger.info(f"📄 D 檔: {FILE_D_CONFIG.path}")
        logger.info(f"📄 E 檔: {FILE_E_CONFIG.path}")
        logger.info("⚠️ 使用 PollingObserver 模式（支援網路路徑）")
        logger.info("=" * 60)
        
        #----------------------------------------------------
        # ✅ 2. 啟動 Beads IPQC 上傳同步服務
        # ----------------------------------------------------
        logger.info("\n[Init] 啟動 Beads IPQC 同步服務...")
        # 如果需要修改路徑，可以傳入 config 字典，否則使用預設值
        try:
            beads_observer = start_monitoring()
        except Exception as e:
            logger.error(f"啟動 Beads 同步服務失敗: {e}")
            beads_observer = None

        if beads_observer:
            logger.info("✅ Beads 同步服務已在背景執行")
        else:
            logger.warning("❌ Beads 同步服務啟動失敗 (請檢查路徑)")

        # 在 Flask 啟動前初始化
        logger.info("\n⚠️ 在 Flask 啟動前執行初始化(原本的 File A-E)")
        initialize_sync_service()
        
        if not state.sync_init_done:
            logger.warning("❌ 警告：初始化未成功，Watchdog 可能無法運行")
        ...
        logger.info("=" * 60)
        logger.info("🔄 服務關閉：執行清理")
        logger.info("=" * 60)
        # ✅ 3. 關閉 Beads 同步服務
        if beads_observer:
            logger.info("Stopping Beads Observer...")
            beads_observer.stop()
            beads_observer.join()
        stop_watch()
