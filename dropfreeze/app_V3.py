...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        print(f"ALL_FIELDS 欄位數: {len(ALL_FIELDS)}")
        print(f"ALL_FIELDS 欄位名: {ALL_FIELDS}")
...
def get_status():
    try:
        ...
        # 決定工站狀態
        last_completed_station = None
        next_station = None
        for station_name, _, photo_col in STATIONS:  # 正確解開 3 元 tuple
            val = row.get(photo_col, "")
            val_str = str(val).strip() if val is not None else ""
            if val_str != "":
                last_completed_station = station_name
            else:
                next_station = station_name
                break

        response = {}
        if last_completed_station is None:
            response["current_station"] = STATIONS[0][0]  # 收藥
        elif next_station is None:
            response["current_station"] = "已完成"
        else:
            response["current_station"] = last_completed_station
        ...
        for field in extra_fields:
            val = row.get(field, "")
            response[field] = str(val).strip() if val is not None else ""
        ...
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"後端例外: {str(e)}"}), 500
