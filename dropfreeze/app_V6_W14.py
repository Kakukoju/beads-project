...
@app.post("/update_ports")
def update_ports():
    """
    寫入 port_1 ~ port_4
    body: { order: "工單號", auth: "36121288", ports: ["Port1","Port2", ...] }
    規則：
    - 只接受 "Port<number>"，大小寫不拘；若傳純數字會自動轉 "PortN"
    - 依序寫入上到下四列；左側 Dispense_Lot_i 若為空或 "#NA" 則該列清空
    """
    try:
        ...
        if not order:
            return jsonify({"status":"fail","msg":"missing order"}), 400
        if auth != "36121288":
            return jsonify({"status":"fail","msg":"auth failed"}), 403
        ...
        for p in ports[:4]:
            ...
            if isinstance(p, str):
                p = p.strip()
                if re.match(r'(?i)^port\d+$', p):
                    norm.append(p.title())  # Port1
                elif p.isdigit():
                    norm.append(f"Port{int(p)}")
                else:
                    return jsonify({"status":"fail","msg":f"bad port value: {p}"}), 400
            else:
                return jsonify({"status":"fail","msg":f"bad port value: {p}"}), 400
        ...
        with get_db_connection() as conn:
            ...
            # 讀出四列 lot
            cur.execute('SELECT Dispense_Lot_1, Dispense_Lot_2, Dispense_Lot_3, Dispense_Lot_4 FROM work_orders WHERE 工單號=?', (order,))
            row = cur.fetchone()
            lots = [ (row[i] if row else None) for i in range(4) ]

            assigns = [None, None, None, None]
            j = 0
            for i in range(4):
                lot = (lots[i] or "").strip()
                if lot and lot != "#NA":
                    assigns[i] = norm[j] if j < len(norm) else None
                    if j < len(norm): j += 1
                else:
                    assigns[i] = None  # 左側沒 lot → 清空

            cur.execute('UPDATE work_orders SET port_1=?, port_2=?, port_3=?, port_4=? WHERE 工單號=?',
                        (assigns[0], assigns[1], assigns[2], assigns[3], order))
            conn.commit()

        return jsonify({"status":"success","order":order,"ports":assigns})
    except Exception as e:
        print("❌ /update_ports 錯誤：", e)
        return jsonify({"status":"fail","msg":str(e)}), 500
