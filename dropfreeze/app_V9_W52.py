...
# === Helper Functions ===
def get_freeze_hours_by_pn(pn):
    """
    從 beads_sync.db / 配藥限制 取得凍乾時間 (hrs)
    """
    if not pn:
        return None
    try:
        with sqlite3.connect(DB_BEADS_SYNC) as conn:
            cur = conn.cursor()
            cur.execute("""
                SELECT 凍乾時間
                FROM 配藥限制
...
def build_pump_resources(rows):
   ...
   # === Ops Helper: 凍乾機狀態查詢 ===
def get_single_freezer_status(freezer_no):
    """
    freezer_no: int (3~12) or 'small'
    """
    now = datetime.now()
    today = now.strftime("%Y/%m/%d")      # 格式: 2025/09/30
    yesterday = (now - timedelta(days=1)).strftime("%Y/%m/%d")

    # 處理顯示名稱與 ID
    if freezer_no == "small":
        freezer_name = "Freeze-Small"
        lyo_id = "Small" # 假設排程表裡小型機叫 Small，若叫 13 請自行調整
    else:
        freezer_name = f"Freezer-{str(freezer_no).zfill(2)}"
        lyo_id = str(freezer_no)
    ...
    with get_db_connection() as conn:
        ...
        # =========================
        # 1️⃣ 優先檢查：是否正在運作中 (查 work_orders)
        # =========================
        cur.execute("""
            SELECT 工單號, PN, 時間_凍乾準備, 時間_凍乾開始, 時間_凍乾結束
            FROM work_orders
            WHERE 日期 = ? AND 凍乾機_1 = ?
        """, (today, freezer_name))
        row = cur.fetchone()
        ...
        if row:
            result["todayUsed"] = True
            result["workOrder"] = row["工單號"] # 如果有跑，直接用這裡的工單

            start = row["時間_凍乾開始"]
            end = row["時間_凍乾結束"]
            prep = row["時間_凍乾準備"]

            # --- 判斷狀態 (Preparing / Running / Finished) ---
            if prep and not start and not end:
        ...
        # =========================
        # 🆕 1.5 若沒在跑，查詢今日排程 (DropletSchedule)
        # =========================
        if result["state"] == "idle":
            # 連線到排程資料庫 (假設您原本有定義 DB_SCHEDULE 常數)
            # 如果排程表跟 work_orders 在同一個 DB，就直接用 conn 即可
            try:
                with sqlite3.connect(DB_SCHEDULE) as conn_sch:
                    ...
                    cur_sch.execute("""
                        SELECT WorkOrder 
                        FROM DropletSchedule 
                        WHERE Date = ? AND Lyophilizer = ?
                    """, (today, lyo_id))
                    
                    row_sch = cur_sch.fetchone()
                    if row_sch:
                        # 找到了！填入預排的工單號
                        result["workOrder"] = row_sch[0]
            except Exception as e:
        ...
        # 2️⃣ 今天沒資料 → 檢查昨天是否排程
        # =========================
        with sqlite3.connect(DB_SCHEDULE) as conn2:
            cur2 = conn2.cursor()
            cur2.execute("""
                SELECT WorkOrder
                FROM DropletSchedule
                WHERE Date = ?
                  AND Lyophilizer = ?
                LIMIT 1
            """, (yesterday, str(freezer_no).zfill(2)))
            sch = cur2.fetchone()
        ...
        # =========================
        cur.execute("""
            SELECT 工單號, PN, 時間_凍乾開始, 時間_凍乾結束
            FROM work_orders
            WHERE 日期 = ?
              AND 凍乾機_1 = ?
        """, (yesterday, freezer_name))
        row_y = cur.fetchone()
        ...
        if row_y["時間_凍乾結束"]:
            return result  # 已結束 → idle

        # --- 跨日仍在跑 ---
        result["state"] = "running"
        result["workOrder"] = row_y["工單號"]

        freeze_hours = get_freeze_hours_by_pn(row_y["PN"])
        if freeze_hours and row_y["時間_凍乾開始"]:
            try:
                t_start = datetime.strptime(row_y["時間_凍乾開始"], "%Y-%m-%d %H:%M:%S")
                remain = freeze_hours * 60 - (now - t_start).total_seconds() / 60
                result["remainMin"] = max(0, int(remain))
            except:
                pass

    return result

def build_freezer_resources():
    freezers = []

    # 03 ~ 12
    for i in range(3, 13):
        freezers.append(get_single_freezer_status(i))

    # 小型凍乾機
    freezers.append(get_single_freezer_status("small"))
...
def ops_freeze_status():
    print("🟢 ops_freeze_status CALLED")

    resources = build_freezer_resources()

    total = len(resources)
    in_use = sum(1 for r in resources if r["state"] == "running")
    free = sum(1 for r in resources if r["state"] in ("idle", "finished"))

    next_release = min(
        [r["remainMin"] for r in resources if r["remainMin"] is not None and r["remainMin"] > 0],
        default=None
    )
