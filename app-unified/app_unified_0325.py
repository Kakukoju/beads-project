...
def api_get_workorder():
    ...
    try:
        with connect() as conn:
            ...
            # --- 策略 B: 如果 PN 找不到，才使用 Name 模糊比對 (備案) ---
            if not selected_qc:
                print(f" ⚠️ PN 查無資料，改用 Name='{base_name}' 進行模糊比對...")
                qc_all = query_db("SELECT * FROM [Liquid form QC]")
                
                for row in qc_all:
                    # 資料庫中的名稱 (例如 cCOR)
                    db_name = norm_name(row.get("Name", "")) 
                    
                    # 邏輯：只要 A 包含 B 或 B 包含 A 就算符合
                    # 例如: "CCOR" in "CCOR R1~R4" -> True
                    if db_name and (db_name == base_name or db_name in base_name or base_name in db_name):
                        selected_qc = row
                        print(f" ✅ 成功透過 Name 模糊比對找到 QC 資料！(Match: {db_name})")
                        break
...
# ==================== 新增路由：搜尋 571 表(資料庫) ====================
import traceback # 用來印出完整錯誤，以防萬一
