...
def api_get_workorder():
    ...
    try:
        with connect() as conn:
            ...
            # ✅ 1. 查詢 [Liquid form QC] - 加入版本欄位
            base_name = norm_name(maker_name or "")
            print(f"📍 標準化名稱: {base_name}")
            
            print(f"\n--- 查詢 [Liquid form QC] ---")
            qc_all = query_db("""
                SELECT Name,[L1-OD],[L2-OD],[L1-起始OD],[L2-起始OD],[懸浮物],[版本]
                FROM [Liquid form QC]
            """)
            ...
            for row in qc_all:
                ...
                if name == base_name:
                    selected_qc = row
                    print(f"✅ 找到 QC 資料，版本: {row.get('版本')}")
                    break
...
def api_search_571_tables():
    ...
    if not work_order:
        return jsonify({"ok": False, "message": "請輸入工單號碼"}), 400

    try:
        with connect() as conn:
            # 1. 找出所有含有 "571" 的 table
            cur = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%571%'")
            tables = [r[0] for r in cur.fetchall()]
            ...
            for t in tables:
                # 檢查是否有工單號碼欄位 (避免錯誤)
                cur_info = conn.execute(f"PRAGMA table_info('{t}')")
                columns = [r[1] for r in cur_info.fetchall()]
                
                if "工單號碼" not in columns:
                    continue

                # 執行搜尋
                sql = f"SELECT * FROM '{t}' WHERE TRIM(工單號碼) LIKE ?"
                cur_data = conn.execute(sql, (f"%{work_order}%",))
                ...
                if rows:
                    ...
                    # 假設工單只會出現在一個主要 Table，找到就跳出 (若需全部顯示可移除 break)
                    break 
            
            if not found_rows:
                return jsonify({"ok": False, "message": "查無此工單資料"}), 404
...
# ==================== 新增：批次更新 571 表格資料 ====================
@app.post("/api/update_571_table")
def api_update_571_table():
    """批次更新 571 表格的資料"""
    try:
        payload = request.get_json(force=True)
        table = payload.get("table", "").strip()
        rows = payload.get("rows", [])
        work_order = payload.get("work_order", "").strip()

        if not table or not rows:
            return jsonify({"ok": False, "message": "缺少 table 或 rows 參數"}), 400
        ...
        with connect() as conn:
            ...
            for row_data in rows:
                # 找出主鍵 (工單號碼 + 料號)
                work_order_val = row_data.get("工單號碼") or row_data.get("工單編號") or row_data.get("WorkOrder")
                material_val = row_data.get("料號")

                if not work_order_val or not material_val:
                    print(f"  ⚠️ 跳過無效資料: {row_data}")
                    continue
                ...
                for col, val in row_data.items():
                    # 跳過主鍵欄位和不存在的欄位
                    if col in ("工單號碼", "工單編號", "WorkOrder", "料號") or col not in existing_cols:
                        continue
                    update_cols.append(f'"{col}" = ?')
                    update_vals.append(val if val != "" else None)
                ...
                sql = f'''
                    ...
                    WHERE "工單號碼" = ? AND "料號" = ?
                '''
                update_vals.extend([work_order_val, material_val])
        ...
        print(f"✅ 成功更新 {updated_count} 筆資料\n")
        return jsonify({
            "ok": True,
            "message": f"已更新 {updated_count} 筆資料",
            "updated_count": updated_count
        })

    except sqlite3.Error as e:
        print(f"❌ Database Error: {e}")
        return jsonify({"ok": False, "message": f"資料庫錯誤: {e}"}), 500
    except Exception as e:
...
def get_quarter_label(date_str):
    ...
    try:
        ...
        return f"{date_obj.year}_Q{quarter}"
    except Exception as e:
        print(f"  ⚠️ 季度解析失敗: {date_str} → {e}")
        return None
    
@app.route('/api/heatmap_usage', methods=['GET'])
def get_heatmap_usage():
    """API：取得藥品使用統計 Heatmap 資料"""
    try:
        mode = request.args.get('mode', 'week')
        
        if mode not in ['week', 'month', 'quarter']:
            return jsonify({"error": "Invalid mode"}), 400
        
        # ✅ 新增：可自定義顯示數量
        top_n = int(request.args.get('top', '10'))  # 預設 Top 10（原本是 8）
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        query = """
            SELECT Marker, WorkOrder, Date
        ...
        if not rows:
            return jsonify({"mode": mode, "markers": [], "buckets": [], "matrix": []})
        ...
        usage_data = defaultdict(lambda: defaultdict(set))
        ...
        for row in rows:
            ...
            # 根據模式計算時間區間
            if mode == 'week':
                bucket = get_week_number(date_str)
            elif mode == 'month':
                bucket = get_month_label(date_str)
            else:
                bucket = get_quarter_label(date_str)
            
            if bucket:
                usage_data[marker][bucket].add(work_order)
                parsed_count += 1
            else:
                # 記錄解析失敗的日期（只記錄前5個）
        ...
        # ✅ 輸出調試信息
        print(f"\n{'='*60}")
        print(f"📊 Heatmap 數據解析報告 (mode={mode})")
        print(f"{'='*60}")
        print(f"  總資料數: {total_rows}")
        print(f"  解析成功: {parsed_count} ({parsed_count/total_rows*100:.1f}%)")
        print(f"  解析失敗: {total_rows - parsed_count}")
        if failed_dates:
            print(f"  失敗範例: {failed_dates}")
        print(f"  唯一 Marker 數: {len(usage_data)}")
        print(f"{'='*60}\n")
        
        # 計算總使用量並排序
        marker_totals = {
            marker: sum(len(work_orders) for work_orders in buckets.values())
            for marker, buckets in usage_data.items()
        }
        
        top_markers = sorted(marker_totals.items(), key=lambda x: x[1], reverse=True)[:top_n]
        markers = [marker for marker, _ in top_markers]
        
        # 取得所有時間區間
        all_buckets = set()
        for buckets in usage_data.values():
            all_buckets.update(buckets.keys())
        
        buckets = sorted(list(all_buckets))
        
        # 限制時間範圍
        if mode == 'week':
            buckets = buckets[-12:] if len(buckets) > 12 else buckets
        elif mode == 'month':
            buckets = buckets[-12:] if len(buckets) > 12 else buckets
        else:
            buckets = buckets[-8:] if len(buckets) > 8 else buckets
        
        # 建立矩陣
        matrix = []
        for marker in markers:
            row = [len(usage_data[marker].get(bucket, set())) for bucket in buckets]
            matrix.append(row)
        ...
        return jsonify({
            ...
            "debug": {
                ...
                "failed_count": total_rows - parsed_count,
                "unique_markers": len(usage_data),
                "sample_failed_dates": failed_dates
            }
        })
