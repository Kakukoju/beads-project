...
def check_suspension(v):
    """(懸浮物) '混濁' = True"""
    return "混濁" in str(v or "")

def check_yes(v):
    """(儲存時避光, 滴定時避光, 滴定時冰浴) 'Yes' = True"""
    return str(v or "").strip().lower() == "yes"

def check_not_room_temp(v):
    """(儲存時冰浴) '室溫' = False, 其他 = True"""
    return str(v or "").strip() != "室溫"

def check_not_no(v):
    """(滴定時攪拌) 'No' = False, 其他 = True"""
    return str(v or "").strip().lower() != "no"
...
def api_get_workorder():
    ...
    try:
        with connect() as conn:
            # ... (find_table_contains_workorder, 查詢 result_rows, 查詢 dispose_lots 都不變) ...
            ...
            if not result_rows:
                print(f"⚠️ 找到表 {target_table}，但查無工單 {work_order} 詳細資料。")
            ...
            # ✅ 2. 查詢 [Liquid form QC] (包含 OD 和 懸浮物)
            qc_all = query_db("""
            ...
            if not selected_qc:
                # 預設值 (包含懸浮物)
                selected_qc = {"L1-OD": None,"L2-OD": None,"L1-起始OD": None,"L2-起始OD": None, "懸浮物": None}

            # ✅ 3. 新增查詢： [滴定條件]
            # (假設 'Name' 欄位也是用 base_name 來對應)
            titration_conditions = query_db("""
                SELECT [儲存時避光], [儲存時冰浴], [滴定時避光], [滴定時冰浴], [滴定時攪拌]
                FROM [滴定條件] WHERE Name = ?
            """, (base_name,))
            
            # 取得第一筆查詢結果，如果沒有則為空 dict
            selected_titration = titration_conditions[0] if titration_conditions else {}
            ...
            for k, qc_col in qc_map.items():
                ...
                val = values[k]
                check_results[k] = {"value": val, "qc_range": selected_qc.get(qc_col), "pass": check_in_range(val, rng)}
            ...
            # ✅ 4. 使用新的輔助函式和來自兩個查詢的資料，動態建立 confirm 物件
            confirm_data = {
                ...
                "stir": check_not_no(selected_titration.get("滴定時攪拌")),
            }
            print(f"🔍 Debug - selected_qc: {selected_qc}")
            print(f"🔍 Debug - selected_titration: {selected_titration}")
            print(f"🔍 Debug - confirm_data: {confirm_data}")
            ...
            data = {
                ...
                "bufferBase": {
                    "L1OD": L1OD, "L2OD": L2OD, "L1StartOD": L1StartOD, "L2StartOD": L2StartOD,
                },
                "qcRanges": {
